...
const SortableItem = ({ item, showDetails, editRecord, deleteTopicData, count }: DndSortableTableItem) => {
	...
	return (
		<tr ref={setNodeRef} style={styleDndTable} className='cursor-pointer' onClick={() => showDetails(item)}>
			...
			<td className='w-40'>{image.includes('.mp4') || image.includes('.mov') ? <video className='border rounded w-32 min-w-[80px] h-16 min-h-[60px]' src={image} controls></video> : <img className='border rounded w-32 min-w-[80px] h-16 min-h-[60px]' src={image} alt='preview' />}</td>
...
const DndTable = ({ dndItemRow, showDetails, editRecord, deleteTopicData, setDisableData, setOrderChanged, setNewOrder }: DndRowData) => {
		...
		<DndContext collisionDetection={closestCenter} onDragEnd={onDragEnd} modifiers={[restrictToVerticalAxis, restrictToWindowEdges]}>
			<SortableContext items={items} strategy={verticalListSortingStrategy}>
				{items?.map((item, index) => (
					<SortableItem key={item.id} item={item} showDetails={showDetails} index={index} editRecord={editRecord} deleteTopicData={deleteTopicData} setDisableData={setDisableData} count={items.length} />
				))}
			</SortableContext>
		</DndContext>
	);
