...
export enum FieldNames {
	...
	fileType = 'fileType',
	acceptNote = 'png, jpeg or mp4, mov files only, 1280 X 720 would look better on devices',
}
...
const AddEditOnboardingModal = ({ onSubmit, onClose, editData, disableData }: AddEditOnboardingData) => {
	...
	useEffect(() => {
		if (editData) {
			...
			APIService.getData(`${URL_PATHS.onboarding}/${editData.uuid}`)
				.then((response) => {
	...
	const getObjOnboarding = () => {
		const objOnboarding: ObjectShape = {
			[FieldNames.title]: Yup.string().required(Errors.PLEASE_ENTER_TITLE).max(60, Errors.TITLE_SHOULD_NOT_EXCEED),
			[FieldNames.androidLandscapeImage]: editData ? Yup.string().notRequired() : Yup.mixed().required(Errors.PLEASE_SELECT_IMAGE_VIDEO_SIMPLIFIED),
			[FieldNames.iphoneLandscapeImage]: editData ? Yup.string().notRequired() : Yup.mixed().required(Errors.PLEASE_SELECT_IMAGE_VIDEO_SIMPLIFIED),
			[FieldNames.ipadLandscapeImage]: editData ? Yup.string().notRequired() : Yup.mixed().required(Errors.PLEASE_SELECT_IMAGE_VIDEO_SIMPLIFIED),
		};
