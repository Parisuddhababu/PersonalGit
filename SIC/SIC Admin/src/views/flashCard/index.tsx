...
const FlashCardCategory = () => {
	...
	const getCategoryDetails = useCallback(() => {
		setLoading(true);
		APIService.getData(`${URL_PATHS.flashCardCategories}/list?page=${filterCategoryData.page}&limit=${filterCategoryData.limit}&sortBy=${filterCategoryData.sortBy}&sortOrder=${filterCategoryData.sortOrder}&search=${filterCategoryData.search}`)
			.then((response) => {
										...
										return (
											<th scope='col' key={colVal.name}>
												<div className='flex items-center'>
													{colVal.name}
													{colVal.sortable && (
														<a onClick={() => onCategoryHandleSort(colVal.fieldName)}>
								...
								{categoryData?.data?.categories.map((data, index) => {
									return (
										...
										>
											...
											<td onClick={rowClickCategoryHandler}>
												<div className='flex items-center'>
