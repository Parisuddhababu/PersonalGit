...
const AddEditStar = ({ onSubmit, onClose, editData }: StartEditProps) => {
	...
	useEffect(() => {
		if (editData) {
			setLoaderStars(true);
			APIService.getData(`${URL_PATHS.starManagement}/get-single/${editData?.uuid}`)
				.then((response) => {
	...
	const getObj = () => {
		const obj: ObjectShape = {
			[FieldNames.moduleName]: Yup.string().trim().required(Errors.PLEASE_ENTER_MODULE_NAME).matches(EXCLUDE_SPECIAL_CHARACTERS, Errors.SPECIAL_CHARACTERS_NOT_ALLOWED),
			[FieldNames.starCount]: Yup.number().required(Errors.PLEASE_ENTER_NUMBER_OF_STARS),
		};
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			if (editData) {
				setLoaderStars(true);
				APIService.patchData(`${URL_PATHS.starManagement}/update`, {
				...
				setLoaderStars(true);
				APIService.postData(`${URL_PATHS.starManagement}/create`, {
					[FieldNames.moduleName]: values.moduleName.trim(),
					[FieldNames.starCount]: values.starCount,
				})
					.then((response) => {
