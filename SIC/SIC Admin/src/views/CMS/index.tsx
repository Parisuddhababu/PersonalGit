...
const CMS = () => {
	...
	const getCMSPagesList = useCallback(() => {
		setLoader(true);
		APIService.getData(`${URL_PATHS.pages}/list?page=${filterCMSData.page}&limit=${filterCMSData.limit}&sortBy=${filterCMSData.sortBy}&sortOrder=${filterCMSData.sortOrder}&keyword=${filterCMSData.search}`)
			.then((response) => {
	...
	const updateCMSPageStatus = (value: boolean) => {
		APIService.patchData(`${URL_PATHS.pages}/${editData?.uuid}/change-status`, {
			isActive: value,
		})
			.then((response) => {
				if (response.status === ResponseCode.success) {
												...
												<div className='flex items-center'>
													...
													{colVal.sortable && (
														<a onClick={() => onHandleSort(colVal.fieldName)}>
															...
															{filterCMSData.sortOrder === 'asc' && filterCMSData.sortBy === colVal.fieldName && getAscIcon()}
															{filterCMSData.sortOrder === 'desc' && filterCMSData.sortBy === colVal.fieldName && getDescIcon()}
														</a>
													)}
												</div>
											</th>
