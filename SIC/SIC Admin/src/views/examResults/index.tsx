...
const ExamResults = () => {
	...
	const getUserExamList = useCallback(() => {
		setLoader(true);
		APIService.getData(`${URL_PATHS.examResultDetail}/result?page=${filterDataExamResult.page}&limit=${filterDataExamResult.limit}&search=${encodeURIComponent(filterDataExamResult.search)}`)
			.then((response) => {
				...
				<div className='p-3 w-full'>
					<div className='flex sm:items-center justify-end mb-3 flex-col sm:flex-row'>
						<div className='sm:mr-auto'>
							<TextInput label='' placeholder='Search here...' name='search' value={filterDataExamResult.search} type='text' onChange={handleSearch} inputIcon={<Search />} />
						</div>
					...
					<div className='overflow-auto w-full'>
						...
						{!records && <p className='text-center'>No Exam Results Found</p>}
					</div>
