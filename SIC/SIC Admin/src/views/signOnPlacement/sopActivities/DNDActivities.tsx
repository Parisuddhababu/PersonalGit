...
const SortableItem = ({ item, deleteQuestionDataModal, onChangeStatus, count, onSelect, selectedActivities, copyRecord }: DndQuestionSortableTableItem) => {
		...
		<DndContext collisionDetection={closestCenter} onDragEnd={onDragEndSopActivity} modifiers={[restrictToVerticalAxis, restrictToWindowEdges]}>
			<SortableContext items={itemsSopActivity} strategy={verticalListSortingStrategy}>
				{itemsSopActivity?.map((item, index) => (
					<SortableItem copyRecord={copyRecord} key={item.id} item={item} index={index} editRecord={editRecord} deleteQuestionDataModal={deleteQuestionDataModal} onChangeStatus={onChangeStatus} count={itemsSopActivity.length} onSelect={onSelect} selectedActivities={selectedActivities} />
				))}
			</SortableContext>
		</DndContext>
	);
