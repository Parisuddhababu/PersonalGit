...
export const ActivityDragAndDropSop = ({ onSubmit, onClose, url, activityUuid }: AddEditActivitiesData) => {
	...
	useEffect(() => {
		if (params.activityId) {
			...
			APIService.getData(`${url}/drag-and-drop/${params.activityId}?isForSop=true`)
				.then((response) => {
	...
	const fileUpdateDNDSop = useCallback((event: React.ChangeEvent<HTMLInputElement>, type: string) => {
		const fileDNDSop = event.target.files?.[0];
		const fileName = event.target.name;
		if (fileDNDSop === undefined) {
			return;
		}
	...
	const getObjDNDSop = () => {
		const obj: ObjectShape = {
			...
			[FieldNames.optionSimplified]: simplifiedOptionalsListDNDSop.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_ENTER_OPTION),
			[FieldNames.optionAudioSimplified]: simplifiedOptionalsListDNDSop.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.PLEASE_SELECT_AUDIO),
			...
			[FieldNames.optionTraditional]: traditionalOptionalsListDNDSop.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_ENTER_OPTION),
			[FieldNames.optionAudioTraditional]: traditionalOptionalsListDNDSop.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.PLEASE_SELECT_AUDIO),
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			...
			if (correctAnswerSimplifiedDNDSop.length && correctAnswerTraditionalDNDSop.length) {
				if (params.activityId) {
					setLoadingDNDSop(true);
					APIService.putData(`${url}/drag-and-drop/${params.activityId}?isForSop=true`, {
						...updatedData,
					})
						.then((response) => {
							if (response.status === ResponseCode.success) {
					...
					setLoadingDNDSop(true);
					APIService.postData(`${url}/drag-and-drop?isForSop=true`, {
						...updatedData,
	 ...
	 */
	useEffect(() => {
		const updatedCorrectAnswers = simplifiedOptionalsListDNDSop.filter((item) => correctAnswerSimplifiedDNDSop?.some((data) => item.optionId === data.optionId))?.map((item) => ({ optionId: item.optionId, option: item.option }));
	 ...
	 */
	useEffect(() => {
		const updatedCorrectAnswers = traditionalOptionalsListDNDSop.filter((item) => correctAnswerTraditionalDNDSop?.some((data) => item.optionId === data.optionId))?.map((item) => ({ optionId: item.optionId, option: item.option }));
	 ...
	 */
	const copyDataDNDSop = useCallback(() => {
		...
		setSimplifiedOptionalsListDNDSop(traditionalOptionalsListDNDSop);
											...
											<tr key={item.optionId}>
												...
												<td className='w-28'>
													<div className='flex gap-3'>
														...
														<DeleteButton data={item} isDeleteStatusModal={deleteTraditionalRecordDNDSop} btnDanger={true} disable={editTraditionalOptionDNDSop?.optionId === item.optionId} />
													</div>
												</td>
											</tr>
										))}
											...
											<tr key={item.optionId}>
												...
												<td className='w-28'>
													<div className='flex gap-3'>
														...
														<DeleteButton data={item} isDeleteStatusModal={deleteSimplifiedRecordDNDSop} btnDanger={true} disable={editSimplifiedOptionDNDSop?.optionId === item.optionId} />
													</div>
												</td>
											</tr>
										))}
