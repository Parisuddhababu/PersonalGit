...
export const ActivityMultipleChoiceSop = ({ onSubmit, onClose, url, activityUuid }: AddEditActivitiesData) => {
	...
	useEffect(() => {
		if (params.activityId) {
			...
			APIService.getData(`${url}/mcq/${params.activityId}?isForSop=true`)
				.then((response) => {
	...
	const handleFileUploadMCQSop = (event: React.ChangeEvent<HTMLInputElement>, type: string, fileName: string) => {
		const fileMCQSop = event.target.files?.[0];
		if (fileMCQSop === undefined) {
			return;
		}
		const fileSizeMCQSop = type === fileTypeEnum.image ? IMAGE_SIZE_MB : AUDIO_SIZE_MB;
		const fileTypesMCQSop = type === fileTypeEnum.image ? [FILE_TYPE.pngType, FILE_TYPE.jpgType, FILE_TYPE.jpegType] : [FILE_TYPE.audioType, FILE_TYPE.wavType];

		if (!fileTypesMCQSop.includes(fileMCQSop.type)) {
			setLoadingMCQSop(false);
			toast.error(type === fileTypeEnum.image ? Errors.PLEASE_ALLOW_JPG_PNG_JPEG_FILE : Errors.PLEASE_ALLOW_MP3_WAV_FILE);
			return;
		}
		if (fileMCQSop.size / 1000 / 1024 > fileSizeMCQSop) {
			toast.error(`Uploaded ${type} size is greater than ${fileSizeMCQSop}MB, please upload ${type} size within ${fileSizeMCQSop}MB or max ${fileSizeMCQSop}MB`);
		}
	...
	const getObjMCQSop = () => {
		const objMCQSop: ObjectShape = {
			...
			[FieldNames.imageSimplified]: imageSimplifiedMCQSop ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.IMAGE_IS_REQUIRED),
			[FieldNames.audioSimplified]: audioSimplifiedMCQSop ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.AUDIO_IS_REQUIRED),

			[FieldNames.optionSimplified]: simplifiedOptionalsListMCQSop.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_ENTER_OPTION),
			[FieldNames.optionAudioSimplified]: simplifiedOptionalsListMCQSop.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.AUDIO_IS_REQUIRED),

			[FieldNames.correctAnswerSimplified]: correctAnswerSimplifiedMCQSop?.optionValue.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_SELECT_CORRECT_OPTION),
			[FieldNames.correctAnswerAudioSimplified]: correctAnswerSimplifiedMCQSop?.optionFileUrl.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.PLEASE_SELECT_CORRECT_AUDIO),
			[FieldNames.imageTraditional]: imageTraditionalMCQSop ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.IMAGE_IS_REQUIRED),
			[FieldNames.audioTraditional]: audioTraditionalMCQSop ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.AUDIO_IS_REQUIRED),

			[FieldNames.optionTraditional]: traditionalOptionalsListMCQSop.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_ENTER_OPTION),
			[FieldNames.optionAudioTraditional]: traditionalOptionalsListMCQSop.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.AUDIO_IS_REQUIRED),

			[FieldNames.correctAnswerTraditional]: correctAnswerTraditionalMCQSop?.optionValue.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_SELECT_CORRECT_OPTION),
			[FieldNames.correctAnswerAudioTraditional]: correctAnswerTraditionalMCQSop?.optionFileUrl.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.PLEASE_SELECT_CORRECT_AUDIO),
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			...
			if (params.activityId) {
				setLoadingMCQSop(true);
				APIService.postData(`${url}/mcq-update/${params.activityId}?isForSop=true`, {
				...
				setLoadingMCQSop(true);
				APIService.postData(`${url}/mcq?isForSop=true`, {
	...
	const copyDataMCQSop = useCallback(() => {
		...
		setCorrectAnswerSimplifiedMCQSop(correctAnswerTraditionalMCQSop);
		setSimplifiedOptionalsListMCQSop(traditionalOptionalsListMCQSop);
