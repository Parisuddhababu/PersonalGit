...
const AddEditSubscriptionModal = ({ onSubmit, onClose, editData, disableFields }: AddEditSubscriptionData) => {
	...
	const getObj = () => {
		const obj: ObjectShape = {
			[FieldNames.subscriptionTypeId]: Yup.string().required(Errors.PLEASE_SELECT_SUBSCRIPTION_ID),
			[FieldNames.name]: Yup.string().trim().required(Errors.PLEASE_ENTER_PLAN_NAME).max(CHARACTERS_LIMIT.name, Errors.MAXIMUM_LIMIT_UPTO_60_CHARACTERS),
			[FieldNames.description]: Yup.string().trim().required(Errors.PLEASE_ENTER_PLAN_DESCRIPTION).max(CHARACTERS_LIMIT.description, Errors.MAXIMUM_LIMIT_UPTO_150_CHARACTERS),
			[FieldNames.price]: Yup.string().required(Errors.PLEASE_ENTER_PLAN_PRICE),
			[FieldNames.allowedChildCount]: Yup.mixed()
				.required(Errors.PLEASE_ENTER_ALLOWED_CHILD)
				.test('limit', Errors.ALLOWED_CHILD_LIMIT, (value) => value && value <= 3),
			[FieldNames.isFreeTrialEnabled]: Yup.string().required(Errors.PLEASE_SELECT_FREE_TRAIL_OPTION),
			[FieldNames.freeTrialDuration]: Yup.number().max(DEFAULT_TRIAL_LIMIT, Errors.MAX_TRIAL_DAYS).required(Errors.PLEASE_ENTER_FREE_TRAIL_DURATION),
			[FieldNames.priceDescription]: Yup.string().trim().required(Errors.PLEASE_ENTER_PRICE_DESCRIPTION).max(CHARACTERS_LIMIT.description, Errors.MAXIMUM_LIMIT_UPTO_150_CHARACTERS),

			[FieldNames.playStoreId]: Yup.string().trim().required(Errors.PLEASE_ENTER_PLAY_STORE_ID),
			[FieldNames.appStoreId]: Yup.string().trim().required(Errors.PLEASE_ENTER_APP_STORE_ID),
		};
				...
				<form className='w-[90vw] md:w-[75vw] lg:w-[60vw]' onSubmit={formik.handleSubmit}>
					<div className={cn(ModelStyle['model__body'])}>
						<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
							...
							<TextInput type='number' placeholder='Number of child' name={FieldNames.allowedChildCount} disabled={disableFields} onChange={formik.handleChange} label='Allowed child count' value={formik.values.allowedChildCount} error={getError(FieldNames.allowedChildCount)} max={MAX_CHILDREN_ALLOWED} required />
							<div className='col-span-2 flex flex-col'>
