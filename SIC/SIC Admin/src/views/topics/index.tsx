...
const Topic = () => {
	...
	const updateTopicStatus = (value: boolean) => {
		APIService.postData(`${URL_PATHS.topic}/change-status/${editTopicData?.uuid}`, {
			isActive: value,
		})
			.then((response) => {
	...
	const submitOrder = useCallback(() => {
		...
		APIService.postData(`${URL_PATHS.topic}/change-order/${params.levelId}`, {
			order: dataToSubmit,
			...
			<div className='mb-3 bg-white shadow-lg rounded-lg overflow-auto'>
				<div className='border-b p-3 flex items-center justify-between'>
					<h6 className='font-medium flex items-center'>
						<Button className='btn-default mr-3' onClick={pathRedirect} title='Back to level'>
							<ArrowSmallLeft />
						</Button>
				...
				<div className='p-3 w-full'>
					<div className='overflow-auto w-full'>
						...
						<table>
							...
							<tbody>{topicData && <DNDTopic dndItemRow={topicData?.data?.topics as TopicDataArr[]} showDetails={showDetails} editRecord={editTopicHandler} onChangeStatus={onChangeStatus} deleteLevelDataModal={deleteTopicModal} setOrderChanged={setOrderChanged} setNewOrder={setNewOrder} addEditKaraoke={onAddKaraoke} />}</tbody>
						</table>
						{orderChanged && (
