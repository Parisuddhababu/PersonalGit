...
const Roles = (props: { onSubmitData: () => void }) => {
	...
	const getRoleDataList = useCallback(() => {
		setLoader(true);
		APIService.getData(`${URL_PATHS.role}/list?page=${filterData.page}&limit=${filterData.limit}&sortBy=${filterData.sortBy}&sortOrder=${filterData.sortOrder}&roleName=${filterData.search}`)
			.then((response) => {
