...
import { getDateFromUTCstamp, getDateTimeFromUTCstamp, sortOrder } from '@utils/helpers';
...
const SubAdmin = () => {
	...
	const getAdminUserDetails = useCallback(() => {
		setLoading(true);
		APIService.getData(`${URL_PATHS.subAdmin}list?page=${filterSubAdminData.page}&limit=${filterSubAdminData.limit}&sortOrder=${filterSubAdminData.sortOrder}&sortBy=${filterSubAdminData.sortBy}&search=${encodeURIComponent(filterSubAdminData.search)}`)
			.then((response) => {
	...
	const updateSubAdminStatus = (value: boolean) => {
		APIService.patchData(`${URL_PATHS.subAdmin}change-status/${editData?.userData.uuid}`, {
			isActive: value,
		})
			.then((response) => {
										...
										return (
											<th scope='col' key={colVal.name}>
												<div className='flex items-center'>
													{colVal.name}
													{colVal.sortable && (
														<a onClick={() => onSubAdminHandleSort(colVal.fieldName)}>
															{(filterSubAdminData.sortOrder === '' || filterSubAdminData.sortBy !== colVal.fieldName) && getDefaultIcon()}
															{filterSubAdminData.sortOrder === 'asc' && filterSubAdminData.sortBy === colVal.fieldName && getAscIcon()}
															{filterSubAdminData.sortOrder === 'desc' && filterSubAdminData.sortBy === colVal.fieldName && getDescIcon()}
														</a>
													)}
												</div>
											</th>
									...
									return (
										...
										>
											...
											<td className='w-40 text-center'>
												<p>
													{getDateFromUTCstamp(data?.userData.createdAt)}
													<small className='block italic opacity-50'>{getDateTimeFromUTCstamp(data?.userData.createdAt)}</small>
												</p>
											</td>
											...
											<td onClick={rowClickSubAdminHandler}>
												<div className='flex items-center'>
