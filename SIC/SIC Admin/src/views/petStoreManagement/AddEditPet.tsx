...
import { IMAGE_NOTE, CHARACTERS_LIMIT, FILE_TYPE, IMAGE_SIZE, PET_POSITION } from '@config/constant';
...
const AddEditPetProductModal = ({ onSubmit, onClose, editData, disabled }: AddEditPetProductsData) => {
	...
	const getObj = () => {
		const obj: ObjectShape = {
			...
			[FieldNames.productName]: Yup.string().trim().required(Errors.PLEASE_ENTER_PRODUCT_NAME).matches(EXCLUDE_SPECIAL_CHARACTERS, Errors.SPECIAL_CHARACTERS_NOT_ALLOWED).max(CHARACTERS_LIMIT.name, Errors.MAXIMUM_LIMIT_UPTO_60_CHARACTERS),
			[FieldNames.stars]: Yup.number().required(Errors.PLEASE_ENTER_NUMBER_OF_STARS),
			[FieldNames.level]: !seasonal && +positionValue !== 3 ? Yup.string().required(Errors.PLEASE_SELECT_LEVEL) : Yup.string().notRequired(),
			[FieldNames.topic]: seasonal && +positionValue !== 3 ? Yup.string().required(Errors.PLEASE_SELECT_TOPIC) : Yup.string().notRequired(),
			[FieldNames.position]: Yup.string().required(Errors.PLEASE_SELECT_POSITION),
		};
					...
					<div className={cn(ModelStyle['model__body'])}>
						...
						<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
							{!seasonal && +positionValue !== healthValue && (
								<div className='mb-4'>
									<Dropdown label='Level' placeholder='Select level' value={formik.values.level} name={FieldNames.level} onChange={formik.handleChange} options={levels} id='levels' error={formik.errors.level && formik.touched.level ? formik.errors.level : ''} required disabled={!!editData || disabled} />
								</div>
							)}
							{seasonal && +positionValue !== healthValue && (
								<div className='mb-4'>
									<Dropdown label='Topic' placeholder='Select topic' value={formik.values.topicId} name={FieldNames.topic} onChange={formik.handleChange} options={topics} id='topics' error={formik.errors.topicId && formik.touched.topicId ? formik.errors.topicId : ''} required disabled={!!editData || disabled} />
								</div>
							)}
							<div className='mb-4'>
