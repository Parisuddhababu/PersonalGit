...
const PetStore = () => {
	...
	const getPetProductsData = useCallback(() => {
		...
		APIService.getData(`${URL_PATHS.petStore}/list?search=${filterProductsData.search}`)
			.then((response) => {
	...
	const updateProductStatus = (value: boolean) => {
		APIService.postData(`${URL_PATHS.petStore}/status-change/${editData?.uuid}`, {
			isActive: value,
		})
			.then((response) => {
				if (response.status === ResponseCode.success) {
								...
								{petStoreData?.data?.map((data, index: number) => {
									return (
										...
										>
											...
											<td onClick={rowClickProductsHandler}>
