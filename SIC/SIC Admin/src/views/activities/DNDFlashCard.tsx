...
const SortableItem = ({ item, editRecord, deleteRecord, editDisable }: DndFlashCardOrderSortableTableItem) => {
	const { id } = item;
	...
	return (
		<>
			<tr key={item.id} ref={setNodeRef} style={style}>
				...
				<td className='w-48'>
					<audio src={item.traditionalAudioUrl} controls></audio>
				</td>

				<td rowSpan={2} className='w-20 text-center'>
					{item.isTextToSpeech ? 'Yes' : 'No'}
				</td>
				<td rowSpan={2} className='w-20 text-center'>
					{item.categoryId ? 'Yes' : 'No'}
				</td>
				<td rowSpan={2} className='w-20 text-center'>
					{item.isFlashCardText ? 'Text' : 'Image'}
				</td>
			...
			<tr key={`${item.id}-traditional`} ref={setNodeRef} style={style}>
				...
				<td className='w-48'>
					<audio src={item.simplifiedAudioUrl} controls></audio>
				</td>
			</tr>
		</>
	);
		...
		<DndContext collisionDetection={closestCenter} onDragEnd={onDragEnd} modifiers={[restrictToVerticalAxis, restrictToWindowEdges]}>
			<SortableContext items={items} strategy={verticalListSortingStrategy}>
				{items?.map((item, index) => (
					<SortableItem key={item.id} item={item} index={index} editRecord={editRecord} deleteRecord={deleteRecord} editDisable={editDisable} />
				))}
			</SortableContext>
		</DndContext>
	);
