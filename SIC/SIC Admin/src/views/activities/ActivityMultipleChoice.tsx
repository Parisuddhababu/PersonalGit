...
export const ActivityMultipleChoice = ({ onSubmit, onClose, url, activityUuid }: AddEditActivitiesData) => {
	...
	useEffect(() => {
		if (params.activityId) {
			setLoadingMCQ(true);
			APIService.getData(`${url}/mcq/${params.activityId}`)
				.then((response) => {
	...
	const handleFileUploadMCQ = (event: React.ChangeEvent<HTMLInputElement>, type: string, fileName: string) => {
		const fileMCQ = event.target.files?.[0];
		if (fileMCQ === undefined) {
			return;
		}
		const fileSizeMCQ = type === fileTypeEnum.image ? IMAGE_SIZE_MB : AUDIO_SIZE_MB;
		const fileTypesMCQ = type === fileTypeEnum.image ? [FILE_TYPE.pngType, FILE_TYPE.jpgType, FILE_TYPE.jpegType] : [FILE_TYPE.audioType, FILE_TYPE.wavType];

		if (!fileTypesMCQ.includes(fileMCQ.type)) {
			setLoadingMCQ(false);
			toast.error(type === fileTypeEnum.image ? Errors.PLEASE_ALLOW_JPG_PNG_JPEG_FILE : Errors.PLEASE_ALLOW_MP3_WAV_FILE);
			return;
		}
		if (fileMCQ.size / 1000 / 1024 > fileSizeMCQ) {
			toast.error(`Uploaded ${type} size is greater than ${fileSizeMCQ}MB, please upload ${type} size within ${fileSizeMCQ}MB or max ${fileSizeMCQ}MB`);
		}
	...
	const getObjMCQ = () => {
		const objMCQ: ObjectShape = {
			...
			[FieldNames.imageSimplified]: imageSimplifiedMCQ ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.IMAGE_IS_REQUIRED),
			[FieldNames.audioSimplified]: audioSimplifiedMCQ ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.AUDIO_IS_REQUIRED),

			[FieldNames.optionSimplified]: simplifiedOptionalsListMCQ.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_ENTER_OPTION),
			[FieldNames.optionAudioSimplified]: simplifiedOptionalsListMCQ.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.AUDIO_IS_REQUIRED),

			[FieldNames.correctAnswerSimplified]: correctAnswerSimplifiedMCQ?.optionValue.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_SELECT_CORRECT_OPTION),
			[FieldNames.correctAnswerAudioSimplified]: correctAnswerSimplifiedMCQ?.optionFileUrl.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.PLEASE_SELECT_CORRECT_AUDIO),
			[FieldNames.imageTraditional]: imageTraditionalMCQ ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.IMAGE_IS_REQUIRED),
			[FieldNames.audioTraditional]: audioTraditionalMCQ ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.AUDIO_IS_REQUIRED),

			[FieldNames.optionTraditional]: traditionalOptionalsListMCQ.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_ENTER_OPTION),
			[FieldNames.optionAudioTraditional]: traditionalOptionalsListMCQ.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.AUDIO_IS_REQUIRED),

			[FieldNames.correctAnswerTraditional]: correctAnswerTraditionalMCQ?.optionValue.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_SELECT_CORRECT_OPTION),
			[FieldNames.correctAnswerAudioTraditional]: correctAnswerTraditionalMCQ?.optionFileUrl.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.PLEASE_SELECT_CORRECT_AUDIO),
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			...
			if (params.activityId) {
				setLoadingMCQ(true);
				APIService.putData(`${url}/mcq-update/${params.activityId}`, {
					...updatedData,
				})
					.then((response) => {
						if (response.status === ResponseCode.success) {
				...
				setLoadingMCQ(true);
				APIService.postData(`${url}/mcq-create`, {
	...
	const copyDataMCQ = useCallback(() => {
		...
		setCorrectAnswerSimplifiedMCQ(correctAnswerTraditionalMCQ);
		setSimplifiedOptionalsListMCQ(traditionalOptionalsListMCQ);
