...
const ActivityStrokeOrder = ({ onSubmit, onClose, url, activityUuid }: AddEditActivitiesData) => {
	...
	useEffect(() => {
		if (params.activityId) {
			APIService.getData(`${url}/stroke-order/${params.activityId}`).then((response) => {
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			...
			!params.activityId
				? APIService.postData(`${url}/stroke-order-create`, submitData)
						.then((response) => {
							if (response.status === ResponseCode.success) {
				...
				: APIService.putData(`${url}/stroke-order-update/${params.activityId}`, submitData)
						.then((response) => {
							if (response.status === ResponseCode.success) {
								toast.success(response.data.message);
							}
					...
					option={[
						{
							...
							onChange: (e) => {
								setProvideSkipStrokeOrder(!provideSkipStrokeOrder);
