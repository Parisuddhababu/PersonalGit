...
const SortableItem = ({ item, deleteActivityDataModal, onChangeStatus, copyRecord, count, onSelect, selectedActivities }: DndActivitySortableTableItem) => {
		...
		<DndContext collisionDetection={closestCenter} onDragEnd={onDragEndActivity} modifiers={[restrictToVerticalAxis, restrictToWindowEdges]}>
			<SortableContext items={itemsActivity} strategy={verticalListSortingStrategy}>
				{itemsActivity?.map((item) => (
					<SortableItem selectedActivities={selectedActivities} copyRecord={copyRecord} key={item.id} item={item} index={item.order} editRecord={editRecord} deleteActivityDataModal={deleteActivityDataModal} onChangeStatus={onChangeStatus} count={itemsActivity.length} onSelect={onSelect} />
				))}
			</SortableContext>
		</DndContext>
	);
