...
const ActivityImage = ({ onSubmit, onClose, url, activityUuid }: AddEditActivitiesData) => {
	...
	useEffect(() => {
		if (params.activityId) {
			...
			APIService.getData(`${url}/images/${params.activityId}`)
				.then((response) => {
	...
	const getObjImage = () => {
		const objImage: ObjectShape = {
			...
			[FieldNames.simplifiedFile]: simplifiedFileImage ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.IMAGE_IS_REQUIRED),
			...
			[FieldNames.simplifiedAudio]: simplifiedMicAllowed && simplifiedFileAudio === '' ? Yup.mixed().required(Errors.PLEASE_SELECT_AUDIO) : Yup.string().notRequired(),

			[FieldNames.traditionalFile]: traditionalFileImage ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.IMAGE_IS_REQUIRED),
			...
			[FieldNames.traditionalAudio]: traditionalMicAllowed && traditionalFileAudio === '' ? Yup.mixed().required(Errors.PLEASE_SELECT_AUDIO) : Yup.string().notRequired(),
		};
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			...
			if (params.activityId) {
				setLoadingImage(true);
				APIService.putData(`${url}/images/${params.activityId}`, submitDataImage)
					.then((response) => {
						if (response.status === ResponseCode.success) {
				...
				setLoadingImage(true);
				APIService.postData(`${url}/images`, submitDataImage)
					.then((response) => {
	...
	const handleFileUploadImage = (event: React.ChangeEvent<HTMLInputElement>, type: string, fileName: string) => {
		...
		const fileSizeImage = type === fileTypeEnum.image ? IMAGE_SIZE_MB : AUDIO_SIZE_MB;
		const fileTypesImage = type === fileTypeEnum.image ? [FILE_TYPE.pngType, FILE_TYPE.jpgType, FILE_TYPE.jpegType] : [FILE_TYPE.audioType, FILE_TYPE.wavType];

		if (!fileTypesImage.includes(fileImage.type)) {
			setLoadingImage(false);
			toast.error(type === fileTypeEnum.image ? Errors.PLEASE_ALLOW_JPG_PNG_JPEG_FILE : Errors.PLEASE_ALLOW_MP3_WAV_FILE);
			return;
		}
		if (fileImage.size / 1000 / 1024 > fileSizeImage) {
			toast.error(`Uploaded ${type} size is greater than ${fileSizeImage}MB, please upload ${type} size within ${fileSizeImage}MB or max ${fileSizeImage}MB`);
		}
		...
		<FormikProvider value={formik}>
			<form className='w-full' onSubmit={formik.handleSubmit}>
				...
				<div className={`${cn(ModelStyle['model__body'])}`}>
					<div className='mb-4'>
						<TextInput placeholder='English Title' name={FieldNames.title} onChange={formik.handleChange} label='English Title' value={formik.values.title} error={getErrorImage(FieldNames.title)} required />
					</div>
						...
						option={[
							{
								...
								onChange: (e) => {
									setProvideSkipImage(!provideSkipImage);
