...
const SortableItem = ({ item, showDetails, editRecord, deleteLevelDataModal, onChangeStatus, count }: DndLevelSortableTableItem) => {
		...
		<DndContext collisionDetection={closestCenter} onDragEnd={onDragEndLevel} modifiers={[restrictToVerticalAxis, restrictToWindowEdges]}>
			<SortableContext items={itemsLevel} strategy={verticalListSortingStrategy}>
				{itemsLevel?.map((item, index) => (
					<SortableItem key={item.id} item={item} showDetails={showDetails} index={index} editRecord={editRecord} deleteLevelDataModal={deleteLevelDataModal} onChangeStatus={onChangeStatus} count={itemsLevel.length} />
				))}
			</SortableContext>
		</DndContext>
	);
