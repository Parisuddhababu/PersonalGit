...
import { AddEditLesson, CreateLesson } from 'src/types/lessons';
...
import { ENGLISH_CODE, SIMPLIFIED_CHINESE_CODE, TRADITIONAL_CHINESE_CODE, CHARACTERS_LIMIT, DEFAULT_REWARD_STARS } from '@config/constant';
...
const AddEditLessonModal = ({ onClose, editData, onSubmit }: AddEditLesson) => {
	...
	useEffect(() => {
		if (editData) {
			...
			APIService.getData(`${URL_PATHS.lesson}/${editData?.uuid}`)
				.then((response) => {
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			...
			if (editData) {
				...
				APIService.putData(`${URL_PATHS.lesson}/${editData?.uuid}`, submitData)
					.then((response) => {
				...
				<FormikProvider value={formik}>
					<form className='w-[95vw] md:w-[75vw] lg:w-[60vw]' onSubmit={formik.handleSubmit}>
						...
						<div className={cn(ModelStyle['model__footer'])}>
							...
							<Button className='btn-default btn-large w-24 justify-center' onClick={onClose}>
								Cancel
							</Button>
						</div>
					</form>
