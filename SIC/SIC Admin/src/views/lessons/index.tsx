...
import AddEditLessonModal from './AddEditLesson';
...
const Lessons = () => {
	...
	const updateLessonStatus = (value: boolean) => {
		APIService.postData(`${URL_PATHS.lesson}/change-status/${editData?.uuid}`, {
			isActive: value,
		})
			.then((response) => {
	...
	const submitOrder = useCallback(() => {
		...
		APIService.postData(`${URL_PATHS.lesson}/change-order`, {
	...
	return (
		<div>
			...
			{isAddEditModel && <AddEditLessonModal onSubmit={getLessonsList} onClose={onClose} editData={editData} />}
		</div>
	);
};
