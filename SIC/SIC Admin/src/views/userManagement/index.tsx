...
import { exportCsv, getDateFromUTCstamp, getDateTimeFromUTCstamp, sortOrder } from '@utils/helpers';
...
const UserManagement = () => {
	...
	const getUserDetails = useCallback(() => {
		setIsLoading(true);
		APIService.getData(`${URL_PATHS.userManagement}?page=${filterDataUserManagement.page}&limit=${filterDataUserManagement.limit}&sortOrder=${filterDataUserManagement.sortOrder}&sortBy=${filterDataUserManagement.sortBy}&fields=phone_number&search=${encodeURIComponent(filterDataUserManagement.search)}`)
			.then((response) => {
	...
	const updateUserDetails = (action: string, value: boolean) => {
		setIsLoading(true);
		APIService.patchData(`${URL_PATHS.userManagement}${editData?.uuid}/status`, {
			action: action,
			value: value,
		})
			.then((response) => {
	...
	const updateUserPassword = (passwordChange: ChangePasswordVariable) => {
		setIsLoading(true);
		APIService.patchData(`${URL_PATHS.userManagement}${editData?.uuid}/change-password`, {
										...
										return (
											<th scope='col' key={colVal.name}>
												<div className='flex items-center'>
													{colVal.name}
													{colVal.sortable && (
														<a onClick={() => onHandleSort(colVal.fieldName)}>
									...
									return (
										...
										>
											...
											<td className='w-40 text-center'>
												<p>
													{getDateFromUTCstamp(data.createdAt)}
													<small className='block italic opacity-50'>{getDateTimeFromUTCstamp(data.createdAt)}</small>
												</p>
											</td>
											...
											<td onClick={rowClickHandler}>
												<div className='flex items-center'>
													...
													<CommonButton data={data} dataHandler={changeUserPassword} isPasswordChange={true} title='Change Password' />
													{data.status !== UserFields.expired && <StatusButton data={data} isChangeStatusModal={onChangeStatus} status={data.status} checked={data.status === UserFields.active} />}
												</div>
											</td>
										</tr>
									);
