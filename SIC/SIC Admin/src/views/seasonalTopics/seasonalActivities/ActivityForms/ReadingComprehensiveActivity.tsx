...
const SeasonalActivityReadingComprehensive = ({ onSubmit, onClose, activityUuid, url }: AddEditSeasonalActivityData) => {
	...
	useEffect(() => {
		if (params?.activityId) {
			...
			APIService.getData(`${url}/reading-paragraph/${params?.activityId}?isForSeasonal=true`)
				.then((response) => {
	...
	const handleFileUploadQASeasonal = (event: React.ChangeEvent<HTMLInputElement>, fileName: string) => {
		const fileQASeasonal = event.target.files?.[0];
		if (fileQASeasonal === undefined) {
			return;
		}
		const fileSize = AUDIO_SIZE_MB;
		const fileTypes = [FILE_TYPE.audioType, FILE_TYPE.wavType];

		if (!fileTypes.includes(fileQASeasonal.type)) {
			setLoadingQASeasonal(false);
			toast.error(Errors.PLEASE_ALLOW_MP3_WAV_FILE);
			return;
		}
		if (fileQASeasonal.size / 1000 / 1024 > fileSize) {
	...
	const onFileUploadHandlerQASeasonal = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
		const fileQASeasonalAudio = event.target.files?.[0];
		const fileName = event.target.name;

		if (fileQASeasonalAudio === undefined) {
			return;
		}
		const fileSize = AUDIO_SIZE_MB;
		const fileTypes = [FILE_TYPE.audioType, FILE_TYPE.wavType];

		if (!fileTypes.includes(fileQASeasonalAudio.type)) {
			toast.error(Errors.PLEASE_ALLOW_MP3_WAV_FILE);
			return;
		}
		if (fileQASeasonalAudio.size / 1000 / 1024 > fileSize) {
			toast.error(`Uploaded audio size is greater than ${fileSize}MB, please upload audio size within ${fileSize}MB or max ${fileSize}MB.`);
			return;
		}
	...
	const getObjQASeasonal = () => {
		const obj: ObjectShape = {
			...
			[FieldNames.phraseSimplified]: Yup.array().of(
				Yup.object().shape({
					[FieldNames.chinese]: recordsListQASeasonal.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_ENTER_PHRASE),
					[FieldNames.english]: recordsListQASeasonal.length ? Yup.string().notRequired() : Yup.string().trim().required(Errors.PLEASE_ENTER_PHRASE).matches(EXCLUDE_SPECIAL_CHARACTERS, Errors.SPECIAL_CHARACTERS_NOT_ALLOWED).max(CHARACTERS_LIMIT.name, Errors.MAXIMUM_LIMIT_UPTO_60_CHARACTERS),
					[FieldNames.pinyin]: recordsListQASeasonal.length ? Yup.string().notRequired() : Yup.string().required(Errors.PLEASE_ENTER_PHRASE),
					[FieldNames.audio]: recordsListQASeasonal.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.AUDIO_IS_REQUIRED),
				})
			),
			[FieldNames.audioSimplified]: audioSimplifiedQASeasonal.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.AUDIO_IS_REQUIRED),
			...
			[FieldNames.audioTraditional]: audioTraditionalQASeasonal.length ? Yup.mixed().notRequired() : Yup.mixed().required(Errors.AUDIO_IS_REQUIRED),
		};
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			...
			if (recordsListQASeasonal.length) {
				if (params.activityId) {
					setLoadingQASeasonal(true);
					APIService.putData(`${url}/reading-paragraph/${params.activityId}?isForSeasonal=true`, { ...updatedData })
						.then((response) => {
							if (response.status === ResponseCode.success) {
					...
					setLoadingQASeasonal(true);
					APIService.postData(`${url}/reading-paragraph?isForSeasonal=true`, { ...updatedData, activityTypeId: values.activityTypeId, topicId: values.topicId, lessonId: values.lessonId })
						.then((response) => {
										...
										(arrayHelpers: MyArrayHelpers) => (
											<div className='mb-4 col-span-2'>
												{formik.values.phraseTraditional?.map((duplicate, index) => {
													...
													return (
														<div key={`${index + 1}`} className='bg-gray-100 rounded border mb-1 p-1 ' draggable onDragStart={() => handleDragStartQASeasonal(index, FieldNames.phraseTraditional)} onDragEnd={handleDragEndQASeasonal} onDragOver={(e) => handleDragOverQASeasonal(e, index, formik.values.phraseTraditional, FieldNames.phraseTraditional)} onDrop={handleDragEndQASeasonal} style={draggedIndexQASeasonal === index ? { border: 'dashed ' } : {}}>
