...
const SortableItem = ({ item, deleteActivityDataModal, onChangeStatus, copyRecord, count, onSelect, selectedActivities }: DndSeasonalActivitySortableTableItem) => {
		...
		<DndContext collisionDetection={closestCenter} onDragEnd={onDragEndSeasonalActivity} modifiers={[restrictToVerticalAxis, restrictToWindowEdges]}>
			<SortableContext items={itemsSeasonalActivity} strategy={verticalListSortingStrategy}>
				{itemsSeasonalActivity?.map((item, index) => (
					<SortableItem copyRecord={copyRecord} key={item.id} item={item} index={index} editRecord={editRecord} deleteActivityDataModal={deleteActivityDataModal} onChangeStatus={onChangeStatus} count={itemsSeasonalActivity.length} onSelect={onSelect} selectedActivities={selectedActivities} />
				))}
			</SortableContext>
		</DndContext>
	);
