...
const SeasonalActivities = () => {
	...
	const getSeasonalActivityList = () => {
		...
		APIService.getData(`${URL_PATHS.seasonalActivities}/list?topicId=${topicId}&lessonId=${lessonId}`)
			.then((response) => {
				if (response.status === ResponseCode.success) {
	...
	const updateSeasonalActivityStatus = (value: boolean) => {
		setLoaderSeasonalActivity(true);
		APIService.patchData(`${URL_PATHS.seasonalActivities}/change-status/${editSeasonalActivityData?.uuid}`, {
			isActive: value,
		})
			.then((response) => {
				if (response.status === ResponseCode.success) {
	...
	const submitOrderSeasonalActivity = useCallback(() => {
		...
		APIService.patchData(`${URL_PATHS.seasonalActivities}/change-order`, {
