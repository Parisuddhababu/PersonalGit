...
const AddEditCountryModal = ({ onSubmit, onClose, editData, disabled }: AddEditCountryData) => {
	...
	const getObj = () => {
		const obj: ObjectShape = {
			[FieldNames.countryCode]: Yup.string().required(Errors.PLEASE_ENTER_COUNTRY_CODE).max(DEFAULT_LIMIT, Errors.COUNTRY_CODE_SHOULD_NOT_EXCEED).matches(ONLY_ALPHABETS, `Numbers and ${Errors.SPECIAL_CHARACTERS_NOT_ALLOWED}`),
			[FieldNames.phoneCode]: Yup.mixed()
				.required(Errors.PLEASE_ENTER_PHONE_CODE)
				.test('number', Errors.PLEASE_ENTER_PHONE_CODE, (value) => (value as number) > 0),
			[FieldNames.countryName]: Yup.string().required(Errors.PLEASE_ENTER_COUNTRY_NAME).max(CHARACTERS_LIMIT.name, Errors.COUNTRY_NAME_SHOULD_NOT_EXCEED).matches(ONLY_ALPHABETS, `Numbers and ${Errors.SPECIAL_CHARACTERS_NOT_ALLOWED}`),
		};
				...
				<form className='w-[90vw] md:w-[75vw] lg:w-[60vw]' onSubmit={formik.handleSubmit}>
					<div className={cn(ModelStyle['model__body'])}>
						<div className='grid grid-cols-2 gap-x-4 gap-y-2'>
							...
							<div className='mb-4'>
								<TextInput type='number' placeholder='Phone code' name={FieldNames.phoneCode} disabled={disabled} onChange={formik.handleChange} label='Phone code' value={formik.values.phoneCode} error={formik.errors.phoneCode && formik.touched.phoneCode ? formik.errors.phoneCode : ''} required />
							</div>
						</div>
					</div>
					{!disabled && (
