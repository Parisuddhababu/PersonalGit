...
import Logout from '@views/login/logoutModel';
...
const Header = ({ showHandler }: DefaultPropsTypes) => {
	...
	const [isLogoutModelShow, setIsLogoutModelShow] = useState<boolean>(false);
	...
	const onClose = () => {
		setIsLogoutModelShow(false);
	};
	const logOutHandler = () => {
		setIsLogoutModelShow(true);
	};
	return (
		<header className='bg-white  z-10 h-[55px] border-b border-[#c8ced3] px-2 pt-2'>
			<nav className=' flex items-center justify-between' aria-label='Global'>
				<div className='flex items-center'>
					...
					<div className=' flex-grow justify-end lg:flex-grow-0 hidden lg:block '>
				...
				<div className=' flex-grow justify-center  lg:hidden'>
					<Link to={`/${ROUTES.app}/${ROUTES.settings}`}>
						<div>
							<img className='h-[25px] w-[103px] mx-auto ' src={logo} alt='' />
						</div>
					</Link>
				</div>
				...
				<div className='flex justify-end'>
					<div className='flex items-center space-x-3'>
						<div>
							<h6 onClick={languageHandler} className='sm:cursor-pointer'>
								{i18n.language}
							</h6>
						</div>
						<div>
						...
						<div>
							...
							{isLogoutModelShow && <Logout onClose={onClose} onSubmit={logOut} />}
						</div>
					</div>
				</div>
			</nav>
