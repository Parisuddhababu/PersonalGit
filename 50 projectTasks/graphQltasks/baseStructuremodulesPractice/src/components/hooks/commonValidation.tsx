...
const commonValidation = () => {
	...
	const firstName = Yup.string()
		...
		.max(20, t('The first name may not be greater than 20 characters.') as string)
		.matches(/^[A-Za-z\s]+$/, t('Please enter valid first name ( only characters )') as string)
	...
	const lastName = Yup.string()
		...
		.max(20, t('The last name may not be greater than 20 characters.') as string)
		.matches(/^[A-Za-z\s]+$/, t('Please enter valid last name (only characters )') as string)
		.test('no-spaces-or-numbers', 'last Name should not be less than 3 characters', (value) => !value || !/[0-9]/.test(value))
		.trim();

	const oldPassword = Yup.string()
		.trim()
		.required(t('Please enter current password') as string)
		.max(10, t('Password must be at most 10 characters') as string)
		.matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/, t('Password must contain 1 lower case, 1 upper case, 1 number, 1 special character ( @ ! $ % ^ & #) & minimum 8 characters') as string);
	...
	const confirmPassword = Yup.string()
		...
		.oneOf([Yup.ref('newPassword'), null], t('Confirm Password should match with new password') as string);
	...
	const password = Yup.string()
		.required(t('Please enter password') as string)
		.max(10, t('Password must be at most 10 characters') as string)
		.matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/, t('Password must contain 1 lower case, 1 upper case, 1 number, 1 special character ( @ ! $ % ^ & #) & minimum 8 characters') as string);
	const forgetEmail = Yup.string()
		.email(t('Invalid email') as string)
		.required(t('Please enter email') as string);
	return {
		firstName,
