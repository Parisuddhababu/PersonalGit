...
const Role = () => {
	const { data, refetch, loading } = useQuery(FETCH_ROLES_DATA);
	...
	return (
		<>
			{/* main div  */}
			<div className=' bg-white shadow-lg rounded-sm  border border-[#c8ced3] '>
				...
				<div className=' p-3 overflow-auto custom-datatable '>
					...
					{!data?.fetchRoles.data?.Roledata.length && <p className='text-center'>{t('No Data')}</p>}
				</div>
				<div className='pl-4 lg:px-6 mb-4 flex items-center justify-between '>
					<span>
						{' '}
