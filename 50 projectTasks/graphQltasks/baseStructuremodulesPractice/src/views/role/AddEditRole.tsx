...
const AddEditRole = ({ onSubmitRole, onClose, roleVal, isRoleEditable, roleObj, isRoleModelShow }: RoleEditProps) => {
	...
	return (
		<>
			<div className={`${isRoleModelShow ? '' : 'hidden'} fixed top-0 left-0 right-0 z-50 h-full  bg-slate-200 bg-opacity-[2px] backdrop-blur-sm `}>
				<div id='defaultModal' tabIndex={-1} data-modal-show={true} aria-hidden='false' className={` pt-10 flex justify-center ${addClass ? '' : 'opacity-0 transform -translate-y-full scale-150 '} transition-all duration-300 `}>
					<div className=' max-h-full '>
						{/* <!-- Modal content --> */}
						<div className='relative bg-white rounded-lg w-auto md:w-[31rem] h-[14rem] mt-4'>
							{/* <!-- Modal header --> */}
