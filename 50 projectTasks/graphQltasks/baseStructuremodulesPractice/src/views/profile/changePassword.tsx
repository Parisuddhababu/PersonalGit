						...
						<div className='p-5 bg-white border-t border-gray-300   dark:bg-gray-800 dark:border-gray-700'>
							<div className='grid grid-cols-1 gap-6 mb-5 md:grid-cols-2 xl:grid-cols-2'>
								<div className='relative'>
									<TextInput id='oldPassword' placeholder={t('Current PassWord') as string} name='oldPassword' type='password' label={t('Current PassWord')} value={formik.values.oldPassword.trim()} onChange={formik.handleChange} error={formik.errors.oldPassword && formik.touched.oldPassword ? formik.errors.oldPassword : ''} required />
								</div>
								<div className='relative'>
									<TextInput id='newPassword' placeholder={t('New Password') as string} type='password' name='newPassword' label={t('New Password')} value={formik.values.newPassword.trim()} onChange={formik.handleChange} error={formik.errors.newPassword && formik.touched.newPassword ? formik.errors.newPassword : ''} required />
								</div>
								<div className='relative'>
									<TextInput id='confirmPassword' name='confirmPassword' type='password' label={t('Confirm Password')} value={formik.values.confirmPassword.trim()} onChange={formik.handleChange} placeholder={t('Confirm Password') as string} error={formik.errors.confirmPassword && formik.touched.confirmPassword ? formik.errors.confirmPassword : ''} required />
								</div>
							</div>
						</div>
					</div>
