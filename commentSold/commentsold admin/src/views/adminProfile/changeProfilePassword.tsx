...
const UserProfilePasswordChange = (): ReactElement => {
	...
	const [changeProfilePassword, { loading }] = useMutation(CHANGE_USERPROFILE_PASSWORD);
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			changeProfilePassword({
				...
				.then((response) => {
					if (response.data?.changeProfilePassword?.meta.statusCode === 200) {
						toast.success(response.data?.changeProfilePassword?.meta.message);
			...
			<div className='card  '>
				...
				<form onSubmit={formik.handleSubmit} className='max-w-full mx-auto'>
					<div className='card-body'>
						...
						<h4 className='mb-3 text-l text-h3 font-medium leading-6'>{('Change Password')}</h4>
						<div className='pt-4 bg-white border-t border-gray-300    '>
							<div className='card-grid-addedit-page md:grid-cols-2'>
								<TextInput error={formik.errors.oldPassword && formik.touched.oldPassword ? formik.errors.oldPassword : ''} btnShowHide={showOldPassword} btnShowHideFun={handleShowpassword} required={true} id='oldPassword' onBlur={OnBlur} type={showOldPassword ? 'text' : 'password'} password={true} label={('Current Password')} value={formik.values.oldPassword} onChange={formik.handleChange} placeholder={('Current Password')} />

								<TextInput btnShowHide={showNewPassword} btnShowHideFun={handleShownewpassword} error={formik.errors.newPassword && formik.touched.newPassword ? formik.errors.newPassword : ''} required={true} onBlur={OnBlur} id='newPassword' type={showNewPassword ? 'text' : 'password'} password={true} label={('New Password')} value={formik.values.newPassword} onChange={formik.handleChange} placeholder={('New Password')} />

								<TextInput error={formik.errors.confirmPassword && formik.touched.confirmPassword ? formik.errors.confirmPassword : ''} btnShowHide={showConfirmPassword} btnShowHideFun={handleconformpassword} required={true} onBlur={OnBlur} id='confirmPassword' type={showConfirmPassword ? 'text' : 'password'} password={true} label={('Confirm Password')} value={formik.values.confirmPassword} onChange={formik.handleChange} placeholder={('Confirm Password')} />
							</div>
						</div>
					</div>
					<div className='card-footer '>
						<div className='flex btn-group '>
							<Button className='btn-primary ' label={('Change Password')} type='submit'>
								<span className='text-white mr-1 w-3.5 h-3.5 inline-block svg-icon'>
									<CheckCircle />
								</span>
							</Button>
							<Button className='btn-warning ' label={('Cancel')} onClick={cancelPassWordHandler}>
