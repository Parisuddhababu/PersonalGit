...
const UpdateProfileForm = (): ReactElement => {
	const { data: getProfile } = useQuery(GET_ADMIN, { fetchPolicy: 'network-only' });
	const navigate = useNavigate();
	const [updateProfile, { loading }] = useMutation(UPDATE_ADMIN_PROFILE, {
		refetchQueries: [{ query: GET_ADMIN }],
	});
	const { updateAdminValidationSchema } = useValidation();
	useEffect(() => {
		if (getProfile) {
			const data = getProfile?.getProfile?.data;
			formik.setValues({
	...
	const errorFunction = (values: { firstName: string; lastName: string }) => {
		if (values.firstName.length > 20 || values.firstName.includes(' ')) {
			toast.error(`${values.firstName.length > 20 ? ('The first name may not be greater than 20 characters.') : ''}${values.firstName.includes(' ') ? ('The first name may only contain letters.') : ''}`);
		}
		if (values.lastName.length > 20 || values.lastName.includes(' ')) {
			toast.error(`${values.lastName.length > 20 ? ('The last name may not be greater than 20 characters.') : ''}${values.lastName.includes(' ') ? ('The last name may only contain letters.') : ''}`);
		}
	};
			...
			<div className='card'>
				...
				<form onSubmit={formik.handleSubmit}>
					<div className='card-body'>
						...
						<h4 className='mb-3 text-l leading-6 text-h3 font-medium '>{('Profile Information')}</h4>
						<div className='pt-4 bg-white border-t border-gray-300'>
							<div className='card-grid-addedit-page md:grid-cols-2'>
								<div>
									<TextInput required={true} onBlur={OnBlur} placeholder={('First Name')} type='text' label={('First Name')} id='firstName' name='firstName' value={formik.values.firstName} onChange={formik.handleChange} error={formik.errors.firstName && formik.touched.firstName ? formik.errors.firstName : ''} />
								</div>
								<div>
									<TextInput required={true} onBlur={OnBlur} placeholder={('Last Name')} type='text' id='lastName' name='lastName' label={('Last Name')} value={formik.values.lastName} onChange={formik.handleChange} error={formik.errors.lastName && formik.touched.lastName ? formik.errors.lastName : ''} />
								</div>

								<div>
									<TextInput placeholder='' disabled type='text' id='email' name='email' label={('Email')} value={formik.values.email} />
								</div>
							</div>
						</div>
					</div>
					<div className='card-footer '>
						<div className='flex btn-group'>
							<Button className='btn-primary ' label={('Update Profile')} type='submit'>
								<span className='text-white mr-1 w-3.5 h-3.5 inline-block svg-icon'>
									<CheckCircle />
								</span>
							</Button>
							<Button className='btn-warning   ' label={('Cancel')} onClick={cancelUpdateHandler}>
