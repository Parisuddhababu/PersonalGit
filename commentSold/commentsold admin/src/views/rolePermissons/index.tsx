...
const RolePermissions = () => {
	...
	const { data: roleData, refetch: refetchRoleData } = useQuery(GET_ROLES_DATA, { variables: { isAll: IS_ALL }, fetchPolicy: 'network-only' });
	const { data: _, refetch: getRolePermissionListById } = useQuery(FETCH_ROLE_PERMISSIONS_BY_ID, { skip: true, fetchPolicy: 'network-only' });
				...
				<div className='w-full h-full '>
					<div className='card'>
						<div className='card-header gap-3 flex-wrap'>
							<Dropdown placeholder={('Select role')} name='role' value={filterRoleData.roleId ? filterRoleData.roleId : ''} onChange={handleChange} options={roleDrpData} id='role' />

							<div>
								<RoleBaseGuard permissions={[PERMISSION_LIST.Permission.createAccess]}>
									<Button className='btn-primary ' label={('Save')} onClick={saveRolePermissionsHandler}>
