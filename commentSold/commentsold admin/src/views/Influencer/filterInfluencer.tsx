...
import { FilterInfluencerProps, InfluencerProps } from '@type/influencer';
import { useFormik } from 'formik';
import React, { ReactElement, useCallback, useEffect } from 'react';
import filterServiceProps from '@components/filter/filter';

const InfluencerFilter = ({ onSearchInfluencer, filterData }: InfluencerProps): ReactElement => {
...
*/
    useEffect(() => {
        const savedInfluencerFilterDataJSON = filterServiceProps.getState('influecer', JSON.stringify(filterData));

        // Parse the JSON data retrieved from local storage
        const savedInfluencerFilterData = JSON.parse(savedInfluencerFilterDataJSON);

        // Set the formik field values using setValues
        formik.setValues(savedInfluencerFilterData || initialValues);
    }, [filterData]);

    const formik = useFormik({
        initialValues,
        onSubmit: (values) => {
            onSearchInfluencer(values);
        },
    });
     ...
     */
    const onInfluencerReset = useCallback(() => {
        formik.resetForm();
        onSearchInfluencer(initialValues);
    }, []);
                    ...
                    <div className='card-grid-filter'>
                        ...
                        <div>
                            <div className='flex items-start justify-end col-span-3 btn-group '>
                                ...
                                <Button className='btn-warning ' onClick={onInfluencerReset} label={('Reset')}>
                                    <span className='svg-icon inline-block h-3.5 w-3.5 mr-1'>
                                        <Refresh />
                                    </span>
                                </Button>
