...
import { FilterSubAdminProps } from '@type/influencer';
import filterServiceProps from '@components/filter/filter';

const FilterSubAdminSearch = ({ onSearchSubAdmin, filterData }: SubAdminProps): ReactElement => {
...
*/
	useEffect(() => {
		const SubAdminFilterDataJSON = filterServiceProps.getState('filtersubadmin', JSON.stringify(filterData));

		// Parse the JSON data retrieved from local storage
		const subAdminFilterData = JSON.parse(SubAdminFilterDataJSON);

		// Set the formik field values using setValues
		formik.setValues(subAdminFilterData || initialValues);
	}, [filterData]);
	 ...
	 */
	const onSubAdminReset = useCallback(() => {
		formik.resetForm();
					...
					<div className='card-grid-filter'>
						...
						<div>
							<div className='flex items-start justify-end col-span-3 btn-group '>
								...
								<Button className='btn-warning ' onClick={onSubAdminReset} label={('Reset')}>
									<span className='svg-icon inline-block h-3.5 w-3.5 mr-1'>
										<Refresh />
									</span>
								</Button>
...
export default FilterSubAdminSearch;
