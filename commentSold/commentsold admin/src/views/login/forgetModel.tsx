					...
					<div className='model-content'>
						...
						<form onSubmit={formik.handleSubmit}>
							<div className='model-body'>
								<TextInput label={('Enter your e-mail address below to reset your password.')} placeholder={('Email')} name='email' onChange={formik.handleChange} value={formik.values.email} error={formik.errors.email} />
							</div>
							<div className='model-footer'>
								<Button type='submit' className='btn-primary  ' disabled={!!(formik.values.email === '' || formik.errors.email)} label={('Submit')} />
								<Button className='  hover:bg-gray-400 btn-gray' onClick={onClose} label={('Close')} />
							</div>
						</form>
					</div>
				</div>
