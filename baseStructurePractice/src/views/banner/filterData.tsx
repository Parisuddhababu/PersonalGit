...
const FilterBannerData = ({ filterBanner }: BannerProps) => {
	...
	return (
		<React.Fragment>
			<form onSubmit={formik.handleSubmit} className='w-full  md:w-auto p-6 mb-5 bg-white rounded-sm shadow-lg border border-[#c8ced3] '>
				<div className='grid grid-cols-1 gap-4 md:grid-cols-3 '>
					...
					<div className=''>
						<Dropdown placeholder={'select status'} name='status' onChange={formik.handleChange} value={formik.values.status as string} options={Status_DropDown_select} id='status' />
					</div>
				</div>
