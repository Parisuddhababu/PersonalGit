...
const Pagination = ({ pageCount, onPageChange, currentPage, hasPreviousPage, hasNextPage }: paginationProps) => {
	...
	return (
		<div className='flex items-center'>
			{/* for first page  */}
			<button className={`btn h-6 w-7 mx-1 ${hasPreviousPage ? '' : ' disabled'}`} onClick={handleFirstPageClick} disabled={!hasPreviousPage}>
				<PreviousIcon />
			</button>
			{/* for pagination  */}
			...
			{/* for last page  */}
			<button className={`btn mx-1 h-6 w-7 ${hasNextPage ? '' : ' disabled'}`} onClick={handleLastPageClick} disabled={!hasNextPage}>
				<NextIcon />
			</button>
		</div>
	);
