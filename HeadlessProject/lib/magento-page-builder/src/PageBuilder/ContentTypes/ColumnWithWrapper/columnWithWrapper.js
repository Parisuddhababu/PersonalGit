...
const ColumnWithWrapper = (props) => {
    ...
    if (mobileImage && typeof window !== 'undefined' && window.matchMedia('(max-width: 767px)').matches) {
        image = mobileImage
    }
    ...
    const arrangeContent = (children) => {
        let wrapperTypes = []
        let nonWrapperTypes = []
        children.map((child, index) => {
            // console.log("ContentType:" + child.props.data.contentType);
            if (
                child.props.data.contentType == 'image' ||
                child.props.data.contentType == 'vimeo-video'
            ) {
                nonWrapperTypes.push(child)
            } else {
                wrapperTypes.push(child)
            }
        })
        return (
            <>
                ...
                {wrapperTypes.length > 0 && (
                    <div className="home-slider-content">{wrapperTypes}</div>
                )}
            </>
        )
    }
...
export default ColumnWithWrapper
