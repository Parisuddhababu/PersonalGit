...
import defaultClasses from './tabs.module.scss'
...
const Tabs = (props) => {
    ...
    const tabHeaders = Children.map(children, (child, index) => {
        return (
            <TabHeader
                key={index}
                className={[child.props.data.cssClasses, classes.header].join(
                    ' '
                )}
                selectedClassName={classes.panelSelected}
            >
                {child.props.data.tabName}
            </TabHeader>
        )
    })
    ...
    const tabPanels = Children.map(children, (child, index) => {
        return (
            <TabPanel
                ...
                selectedClassName={['selected', classes.panelSelected].join(
                    ' '
                )}
            >
                {child}
