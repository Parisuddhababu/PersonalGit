...
export class BannerComponent implements OnInit , OnDestroy{
  ...
  ngOnInit(): void {
    const titleControlSubscription =  this.form.get('bannerTitleControl')?.valueChanges.subscribe((value) => {
      this.fetchBanners(value!, this.sortBy, this.sortOrder, this.page, this.limit);
    });
  ...
  /*banner fetch method */
  fetchBanners(title: string, sortBy: string, sortOrder: string, page: number, limit: number): void {
    const fetchBannersSubscription = this.bannerService.fetchBannerData(title, sortBy, sortOrder, page, limit).subscribe({
      next: (res) => {
  ...
  updateSorting(): void {
    ...
    this.fetchBanners(title, this.sortBy, this.sortOrder, this.page, this.limit);
  }
/*banner pagination method */
  ...
  onDeleteBanner(uuid: string) {
    if (confirm('Are you sure you want to delete?')) {
      const deleteBannerSubscription = this.bannerService.DeleteBannerData(uuid).subscribe({
        next: (response) => {
  ...
  toggleActive(banner: {uuid:string;status:number}) {
    const toggleActiveSubscription =  this.bannerService.UpdateBannerService(banner.uuid, banner.status).subscribe({
      next: (response) => {
