...
@Component({
  ...
  providers: [BannerService, UploadImageService],
})
export class AddBannerComponent {
  ...
  handleImageChange(event: Event): void {
    ...
    if (
      ...
      this.error =
        'File must be (jpeg/png), File size must be 2MB or below, and aspect ratio must be either 1:1 or 16:9';
    }
  }
  ...
  onSubmit() {
    if (this.paramId) {
      console.log(this.form.value.status, 'status value');
      this.bannerService
        .UpdateBannerData(
          this.paramId,
        ...
        .subscribe(
          ...
          (error) => {
            console.log(error, 'error while update banner');
          }
        );
    } else {
      this.bannerService
        ...
        .subscribe(
          ...
          (error) => {
            console.log(error, 'error while creating banner');
          }
        );
    }
  }
