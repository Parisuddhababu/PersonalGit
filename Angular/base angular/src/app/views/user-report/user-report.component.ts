...
export class UserReportComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sortParam = this.filterService.getSingleState('reportSortParam', this.sortParam);
    this.sortOrder = this.filterService.getSingleState('reportSortOrder', this.sortOrder);
    this.pageNumber = this.filterService.getSingleState('reportPageNo', this.pageNumber);
    ...
    this.reportService
      ...
      .subscribe(
        (resp) => {
          ...
          this.reportList = resp.data['original'].data;
  ...
  rerender(goFirstPage, isDelete = false): void {
    ...
    if (isDelete && this.pageNumber && this.reportList.length === 1) {
