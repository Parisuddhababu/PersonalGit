...
const OurStoryImages2 = ({ data }: IOurStoryImagesProps) => {
  const [legthOfSpecificArrayChunk] = useState<number>(4);
  ...
  return (
    <>
      <Head>
        <link rel="stylesheet" href={getTypeBasedCSSPath(data?.type, CSS_NAME_PATH.productSlider3items2)} />
      </Head>

      {data?.ourstory_image?.length > 0 ? <section className="product_slider">
        {hideLeftButton && (
          <button
            type="button"
...
export default OurStoryImages2;
