...
const SimilarProductView1 = (props: ISimilarViewProps) => {
  ...
  const getCondition = (data: ISimilarProductsProps) => {
    ...
    else return !wishListData?.some((ele) => ele == data?.product_id ? data?.product_id : data?.product?._id);
  };
  ...
  const addtoWishList = async (data: ISimilarProductsProps) => {
    ...
    if (getCondition(data)) {
      ...
      const response = await adddtoWishListProduct(wishListData as IAddtoWishList);
      ...
      const response = await removeWishListProduct(wishListData as IRemoveWishList);
      // @ts-ignore
          ...
          <section className="similar-products-section">
            <div className="container">
              ...
              <div className="product-slider">
                ...
                {similarList?.map((product: ISimilarProductsProps, pInd: number) => (
                  <ProductBox
                    product={{
