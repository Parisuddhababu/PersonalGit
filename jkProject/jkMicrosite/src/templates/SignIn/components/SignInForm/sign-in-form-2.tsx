...
import { IMobileAndEmailData, ISignInForm, ISigninFormProps, ISignInSocialState } from "@templates/SignIn/components/SignInForm/index";
import usePostFormDataHook from "@components/Hooks/postFormDataRegisterHook";
import APICONFIG from "@config/api.config";
import { useToast } from "@components/Toastr/Toastr";
import { IsBrowser, emailLowerCase, setDataInCookies, setDataInLocalStorage } from "@util/common";
import { Action_UserDetails, updateCartCounter, updateWishListCounter } from "src/redux/signIn/signInAction";
...
import OtpForm1 from "./otp-form-1";
...
import MobileAndEmailForm from "./mobile-and-email-form";

const SignInFormSection2 = ({ guestUser = false, reloadPage = false, SignInformData, onComplete }: ISigninFormProps) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <>
      ...
      <section className="sign-in-sec">
        <div className="container">
          {Object.entries(otpData).length ? (
            ...
            <div className="guest-sign-in">
              <SignUpFormSection2 isGuest={true} />
            </div>
          ) : (
            <div className="sign-in-wrap">
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <i
                          className={
                            passwordType == "password"
                              ? "jkms2-eye-off hide-show-icon active"
                              : "jkms2-eye-fill hide-show-icon active"
                          }
                          onClick={() =>
                            setPasswordType(
                ...
                {ispageOne && guestUser && !isB2BUser && (
                  <Link href="#">
                    <a className="create-account" onClick={() => setSignUpFormForUser(true)}>
            ...
            <p>
              New User ?{" "}
              <a href="sign-up.html" className="create-new-account">
                Create an Account
              </a>
            </p> */}
              </form>
        ...
        <Link href={SignInformData?.link || ""}>
          <a className="sign-in-bg">
            <CustomImage
              src={SignInformData?.banner_image?.path || IMAGE_PATH.productBannerJpg}
              alt={SignInformData?.banner_title}
              title={SignInformData?.banner_title}
              height=""
              width=""
            />
          </a>
      ...
      <Head>
        <link rel="stylesheet" href={APPCONFIG.STYLE_BASE_PATH_COMPONENT + CSS_NAME_PATH.signIn2 + ".min.css"} />
      </Head>
    </>
  );
};

export default SignInFormSection2;
