...
const ContactUsAddressList2 = () => {
  ...
  const [defautCountry, setDefaultCountry] = useState()
  const {
    control,
    formState: { errors },
    handleSubmit,
  ...
  const getCountry = async () => {
    ...
    await pagesServices
      ...
      .then((result) => {
        ...
        setIsLoading(false);
        if (Cookies.get("country_id")) {
          const selectedData = result?.data?.country_list.find((ele: any) => ele?._id == Cookies.get("country_id"))
          if (selectedData?.country_code && selectedData?.country_flag) {
            setDefaultCountry(selectedData)
          }
        }
      })
      .catch((error) => {
  ...
  return (
    <>
      ...
      <div className="d-col right-side">
        <div className="box">
          <div className="form-wrap">
            <form onSubmit={handleSubmit(submitAddress)}>
              <div className="d-row">
                ...
                <div className="d-col d-col-12 mb-0">
                  <button
                    type="submit"
                    disabled={isDisabled}
                    className="btn btn-primary btn-small mb-0"
                  >
                    Submit
                  </button>
                </div>
...
export default ContactUsAddressList2;
