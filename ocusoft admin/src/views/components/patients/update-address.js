...
const UpdateAddress = ({ close, id, completed, address }) => {
    ...
    const [defaultValues] = useState({
        ...
        country_id: address?.country?.country_id,
        state_id: address?.state?.state_id,
        city_id: address?.city?.city_id,
        pincode: address?.pincode,
        is_default_shipped: address?.is_default_shipped,
    ...
    const [checkbox, setCheckbox] = useState({
        is_default_shipped: address?.is_default_shipped === 1,
        is_default_billing: address?.is_default_billing === 1,
    })
    ...
    const onSubmit = async (data) => {
        setIsLoading(true)
        API.updateAddress(updateAddress, data, true, address?._id);
    };
    ...
    const getCityData = async () => {
        API.getCityList(getCity, { country_id: country?.country_id, state_id: country?.state_id }, true);
    };
                ...
                <form name="subMasterFrm" onSubmit={handleSubmit(onSubmit)}>
                    <div className="card">
                        ...
                        <div className="card-footer">
                            <button type="submit" className="btn btn-primary mb-2 mr-2"><CIcon icon={cilCheck} />Update</button>
                        </div>
                    </div>
                </form>
            </CModalBody>
