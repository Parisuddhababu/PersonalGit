...
const SortableSeasonalTopicItem = ({ item, showDetails, editRecord, deleteLevelDataModal, onChangeStatus, addEditKaraokeSeasonal, count }: DndSeasonalTopicSortableTableItem) => {
		...
		<tr ref={setNodeRef} style={styleSeasonalTopic} className='cursor-pointer' onClick={() => showDetails(item)}>
			...
			<td className='cursor-default' onClick={(e) => e.stopPropagation()}>
				<div className='flex justify-center items-center'>
					...
					<RoleBaseGuard permissions={[permissionsArray.SEASONAL_TOPIC_MANAGEMENT.EditAccess]}>
						<EditButton data={item} editRecord={editRecord} />
					</RoleBaseGuard>
					<RoleBaseGuard permissions={[permissionsArray.SEASONAL_TOPIC_MANAGEMENT.DeleteAccess]}>
						<DeleteButton data={item} isDeleteStatusModal={deleteLevelDataModal} />
					</RoleBaseGuard>
					<RoleBaseGuard permissions={[permissionsArray.SEASONAL_TOPIC_MANAGEMENT.ChangeStatusAccess]}>
		...
		<DndContext collisionDetection={closestCenter} onDragEnd={onDragEndSeasonalTopic} modifiers={[restrictToVerticalAxis, restrictToWindowEdges]}>
			<SortableContext items={itemsSeasonalTopic} strategy={verticalListSortingStrategy}>
				{itemsSeasonalTopic?.map((item, index) => (
					<SortableSeasonalTopicItem key={item.id} item={item} showDetails={showDetails} index={index} editRecord={editRecord} deleteLevelDataModal={deleteLevelDataModal} onChangeStatus={onChangeStatus} addEditKaraokeSeasonal={addEditKaraokeSeasonal} count={itemsSeasonalTopic.length} />
				))}
			</SortableContext>
		</DndContext>
	);
