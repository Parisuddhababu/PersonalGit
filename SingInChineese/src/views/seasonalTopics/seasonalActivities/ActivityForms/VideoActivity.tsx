...
const SeasonalActivityVideo = ({ onSubmit, onClose, url, activityUuid, toggleSeasonalActivity, isMoving, topicId, lessonId }: AddEditSeasonalActivityData) => {
			...
			APIService.getData(`${url}/${params.activityId}?isForSeasonal=true`)
				.then((response) => {
					if (response.status === ResponseCode.success) {
						...
						setShowInTeacherPanelTraditionalSeasonal(activityData?.isTeacherPanelTraditional);
						setShowInTeacherPanelSimplifiedSeasonal(activityData?.isTeacherPanelSimplified);
					}
		...
		onSubmit: (values) => {
			const submitData: VideoActivitySubmitData = {
				...
				activityData: {
					...
					isTeacherPanelTraditional: showInTeacherPanelTraditionalSeasonal,
					isTeacherPanelSimplified: showInTeacherPanelSimplifiedSeasonal,
				},
				isSkippable: provideSkipVideoSeasonal,
				isForSeasonal: true,
			};
	...
	const fileUpdateVideoSeasonal = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
		const fileVideoSeasonal = event.target.files?.[0] as File;
		typeValidationAudio(fileVideoSeasonal, fileTypesVideo, Errors.UNSUPPORTED_FILE_FORMAT);
					...
					<div className='flex flex-col md:flex-row gap-3 mb-4'>
						<div className='w-full'>
							...
							<CheckBox
								option={[
									{
										id: 'showInTeacherPanelTraditionalSeasonal',
						...
						<div className='w-full'>
							...
							<CheckBox
								option={[
									{
										id: 'showInTeacherPanelSimplifiedSeasonal',
										name: 'Show in teacher panel',
										value: 'Show in teacher panel',
						...
						option={[
							{
								...
								onChange: () => {
									setProvideSkipVideoSeasonal((prev) => !prev);
								},
							},
						]}
					/>
