...
import { SetFile } from '@views/activities/ActivityImage';
...
const SeasonalActivityImage = ({ onSubmit, onClose, url, activityUuid, toggleSeasonalActivity, isMoving, topicId, lessonId }: AddEditSeasonalActivityData) => {
	...
	const defaultSeasonalSpeechToTextTraditional = { [FieldNames.chinese]: '' };
	const defaultSeasonalSpeechToTextSimplified = { [FieldNames.chinese]: '' };
	...
	useEffect(() => {
		if (params?.activityId) {
			...
			APIService.getData(`${url}/${activityPaths.image}/${params?.activityId}?${activityPaths.isForSeasonal}`)
				.then((response) => {
					if (response.status === ResponseCode.success) {
						...
						formik.setFieldValue(FieldNames.simplifiedSpeechToTextDataArray, activityData?.simplifiedSpeechToTextDataArray ?? [defaultSeasonalSpeechToTextSimplified]);
						...
						formik.setFieldValue(FieldNames.traditionalSpeechToTextDataArray, activityData?.traditionalSpeechToTextDataArray ?? [defaultSeasonalSpeechToTextTraditional]);
	...
	const initialValues: CreateImageActivityForm = {
		...
		[FieldNames.simplifiedSpeechToTextDataArray]: [defaultSeasonalSpeechToTextSimplified],
		...
		[FieldNames.traditionalSpeechToTextDataArray]: [defaultSeasonalSpeechToTextTraditional],
	...
	const getObjImageSeasonal = () => {
		const objImageSeasonal: ObjectShape = {
			...
			[FieldNames.simplifiedFile]: simplifiedFileImageSeasonal ? mixedNotRequired() : mixedRequired(Errors.IMAGE_IS_REQUIRED),
			...
			[FieldNames.simplifiedSpeechToTextDataArray]: Yup.array().of(
				Yup.object().shape({
					[FieldNames.chinese]: simplifiedMicAllowedSeasonal ? stringTrim(Errors.PLEASE_ENTER_TEXT) : stringNotRequired(),
				})
			),
			[FieldNames.simplifiedAudio]: simplifiedMicAllowedSeasonal && simplifiedFileAudioSeasonal === '' ? mixedRequired(Errors.PLEASE_SELECT_AUDIO) : stringNotRequired(),

			[FieldNames.traditionalFile]: traditionalFileImageSeasonal ? mixedNotRequired() : mixedRequired(Errors.IMAGE_IS_REQUIRED),
			...
			[FieldNames.traditionalSpeechToTextDataArray]: Yup.array().of(
				Yup.object().shape({
					[FieldNames.chinese]: traditionalMicAllowedSeasonal ? stringTrim(Errors.PLEASE_ENTER_TEXT) : stringNotRequired(),
				})
			),
			[FieldNames.traditionalAudio]: traditionalMicAllowedSeasonal && traditionalFileAudioSeasonal === '' ? mixedRequired(Errors.PLEASE_SELECT_AUDIO) : stringNotRequired(),
		};
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			const submitDataImageSeasonal: ImageActivitySubmitData = {
				...
				isSkippable: provideSkipImageSeasonal,
				isForSeasonal: values.isForSeasonal,
			};
			if (params?.activityId) {
				setLoadingImageSeasonal(true);
				APIService.putData(`${url}/${activityPaths.image}/${params?.activityId}`, submitDataImageSeasonal)
					.then((response) => {
						if (response.status === ResponseCode.success) {
				...
				setLoadingImageSeasonal(true);
				APIService.postData(`${url}/${activityPaths.image}`, submitDataImageSeasonal)
					.then((response) => {
	...
	const traditionalSeasonalSpeechToText = () => {
	...
	const copyDataImageSeasonal = useCallback(() => {
		formik.setFieldValue(FieldNames.simplifiedTitle, traditionalSeasonalData());
		formik.setFieldValue(FieldNames.simplifiedSpeechToTextDataArray, traditionalSeasonalSpeechToText());
	...
	const handleFileUploadImageSeasonal = (event: React.ChangeEvent<HTMLInputElement>, type: string, fileName: string) => {
		const fileImageSeasonal = event.target.files?.[0] as File;
		typeValidation(fileImageSeasonal, type);
	...
	const addMoreTraditionalSpeechToTextSeasonal = useCallback(() => {
	...
	const addMoreSimplifiedSpeechToTextSeasonal = useCallback(() => {
		...
		<FormikProvider value={formik}>
			<form className='w-full' onSubmit={formik.handleSubmit}>
				...
				<div className={`${cn(ModelStyle['model__body'])}`}>
					<div className='mb-4'>
						<TextInput placeholder='English Title' name={FieldNames.title} onChange={formik.handleChange} label='English Title' value={formik.values.title} error={getErrorImageSeasonal(FieldNames.title) as string} required />
					</div>
					<div className='flex flex-col md:flex-row gap-3 mb-4'>
						<div className='rounded border w-full'>
							...
							<div className='grid grid-cols-1 gap-x-4 p-3'>
								...
								<div>
									<div className='font-medium'>Speech to Text {traditionalMicAllowedSeasonal && <span className='text-error'>*</span>}</div>
									<div className='p-4 rounded border bg-gray-50 mb-4'>
										<FieldArray
												...
												(arrayHelpers: MyArrayHelpers) => (
													<div className='mb-4 col-span-2'>
														{formik.values.traditionalSpeechToTextDataArray?.map((_, index) => {
															const speechToTextTraditionalSeasonal = `traditionalSpeechToTextDataArray[${index}].chinese`;
															return (
																<div key={`${index + 1}`} className='bg-gray-100 rounded border mb-1 p-1 '>
																	<div className='flex space-x-3 items-start'>
																		<div className=' w-full'>
																			<TextInput placeholder='Chinese' name={speechToTextTraditionalSeasonal} onChange={formik.handleChange} label='Chinese' value={formik.values.traditionalSpeechToTextDataArray[index].chinese} error={getIn(formik.touched, speechToTextTraditionalSeasonal) && getIn(formik.errors, speechToTextTraditionalSeasonal) ? getIn(formik.errors, speechToTextTraditionalSeasonal) : ''} required={traditionalMicAllowedSeasonal} />
																		</div>
																		<div className='mt-7'>
														...
														<Button className='btn-primary h-[38px] mt-3' onClick={addMoreTraditionalSpeechToTextSeasonal}>
															<Plus className='mr-3' /> Add More
														</Button>
													</div>
												),
						...
						<div className='rounded border w-full'>
							...
							<div className='grid grid-cols-1 gap-x-4 p-3'>
								...
								<div>
									<div className='font-medium'>Speech to Text {simplifiedMicAllowedSeasonal && <span className='text-error'>*</span>}</div>
									<div className='p-4 rounded border bg-gray-50 mb-4'>
										<FieldArray
												...
												(arrayHelpers: MyArrayHelpers) => (
													<div className='mb-4 col-span-2'>
														{formik.values.simplifiedSpeechToTextDataArray?.map((_, index) => {
															const speechToTextSimplifiedSeasonal = `simplifiedSpeechToTextDataArray[${index}].chinese`;
															return (
																<div key={`${index + 1}`} className='bg-gray-100 rounded border mb-1 p-1 '>
																	<div className='flex space-x-3 items-start'>
																		<div className=' w-full'>
																			<TextInput placeholder='Chinese' name={speechToTextSimplifiedSeasonal} onChange={formik.handleChange} label='Chinese' value={formik.values.simplifiedSpeechToTextDataArray[index].chinese} error={getIn(formik.touched, speechToTextSimplifiedSeasonal) && getIn(formik.errors, speechToTextSimplifiedSeasonal) ? getIn(formik.errors, speechToTextSimplifiedSeasonal) : ''} required={simplifiedMicAllowedSeasonal} />
																		</div>
																		<div className='mt-7'>
														...
														<Button className='btn-primary h-[38px] mt-3' onClick={addMoreSimplifiedSpeechToTextSeasonal}>
															<Plus className='mr-3' /> Add More
														</Button>
													</div>
												),
						...
						option={[
							{
								...
								onChange: () => {
									setProvideSkipImageSeasonal((prev) => !prev);
								},
							},
						]}
					/>
