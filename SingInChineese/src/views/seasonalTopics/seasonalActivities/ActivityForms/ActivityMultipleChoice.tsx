...
export const SeasonalActivityMultipleChoice = ({ onSubmit, onClose, url, activityUuid, toggleSeasonalActivity, isMoving, topicId, lessonId }: AddEditSeasonalActivityData) => {
	...
	useEffect(() => {
		if (params.activityId) {
			...
			APIService.getData(`${url}/${activityPaths.mcq}/${params.activityId}?${activityPaths.isForSeasonal}`)
				.then((response) => {
	...
	const handleFileUploadMCQSeasonal = (event: React.ChangeEvent<HTMLInputElement>, type: string, fileName: string) => {
		const fileMCQSeasonal = event.target.files?.[0] as File;
		typeValidation(fileMCQSeasonal, type);
	...
	const getObjMCQSeasonal = () => {
		const objMCQSeasonal: ObjectShape = {
			...
			[FieldNames.imageSimplified]: imageSimplifiedMCQSeasonal ? mixedNotRequired() : mixedRequired(Errors.IMAGE_IS_REQUIRED),
			[FieldNames.audioSimplified]: audioSimplifiedMCQSeasonal ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
			[FieldNames.optionSimplified]: simplifiedOptionalsListMCQSeasonal.length ? stringNotRequired() : Yup.string().trim().required(Errors.PLEASE_ENTER_OPTION),
			[FieldNames.optionAudioSimplified]: simplifiedOptionalsListMCQSeasonal.length ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
			[FieldNames.correctAnswerSimplified]: correctAnswerSimplifiedMCQSeasonal?.optionValue.length ? stringNotRequired() : stringTrim(Errors.PLEASE_SELECT_CORRECT_OPTION),
			[FieldNames.correctAnswerAudioSimplified]: correctAnswerSimplifiedMCQSeasonal?.optionFileUrl.length ? mixedNotRequired() : mixedRequired(Errors.PLEASE_SELECT_CORRECT_AUDIO),
			[FieldNames.imageTraditional]: imageTraditionalMCQSeasonal ? mixedNotRequired() : mixedRequired(Errors.IMAGE_IS_REQUIRED),
			[FieldNames.audioTraditional]: audioTraditionalMCQSeasonal ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
			[FieldNames.optionTraditional]: traditionalOptionalsListMCQSeasonal.length ? stringNotRequired() : Yup.string().trim().required(Errors.PLEASE_ENTER_OPTION),
			[FieldNames.optionAudioTraditional]: traditionalOptionalsListMCQSeasonal.length ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
			[FieldNames.correctAnswerTraditional]: correctAnswerTraditionalMCQSeasonal?.optionValue.length ? stringNotRequired() : stringTrim(Errors.PLEASE_SELECT_CORRECT_OPTION),
			[FieldNames.correctAnswerAudioTraditional]: correctAnswerTraditionalMCQSeasonal?.optionFileUrl.length ? mixedNotRequired() : mixedRequired(Errors.PLEASE_SELECT_CORRECT_AUDIO),
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			...
			if (params.activityId) {
				setLoadingMCQSeasonal(true);
				APIService.postData(`${url}/${activityPaths.mcqUpdate}/${params.activityId}?${activityPaths.isForSeasonal}`, {
				...
				setLoadingMCQSeasonal(true);
				APIService.postData(`${url}/${activityPaths.mcq}?${activityPaths.isForSeasonal}`, {
	...
	const copyDataMCQSeasonal = useCallback(async () => {
		const traditionalMCQOptionsTranslateSeasonal = await Promise.all(
			traditionalOptionalsListMCQSeasonal.map(async (item) => {
				return {
					...item,
		...
		setCorrectAnswerSimplifiedMCQSeasonal({
			...correctAnswerTraditionalMCQSeasonal,
			optionValue: await translateText(correctAnswerTraditionalMCQSeasonal.optionValue, SIMPLIFIED_CHINESE_CODE, TRADITIONAL_CHINESE_CODE).then((data: string) => {
				return data ?? data;
			}),
		});
		setSimplifiedOptionalsListMCQSeasonal(traditionalMCQOptionsTranslateSeasonal);
