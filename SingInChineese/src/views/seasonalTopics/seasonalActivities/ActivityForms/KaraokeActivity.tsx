...
import { generateUuid, translateText, typeValidationAudio } from '@utils/helpers';
...
import { fileTypesVideo } from '@views/activities/ActivityVideo';
...
const ActivityKaraokeSeasonal = ({ onSubmit, onClose, editData }: ActivityKaraokeSeasonalData) => {
	...
	const [showInTeacherPanelTraditionalSeasonal, setShowInTeacherPanelTraditionalSeasonal] = useState<boolean>(false);
	const [showInTeacherPanelSimplifiedSeasonal, setShowInTeacherPanelSimplifiedSeasonal] = useState<boolean>(false);
	...
	const initialValues: CreateKaraokeActivity = {
		...
		isForSeasonal: true,
		isTeacherTraditionalFile: false,
		isTeacherSimplifiedFile: false,
	};
	...
	useEffect(() => {
		if (editKaraokeSeasonal) {
			...
			setShowInTeacherPanelTraditionalSeasonal(karaoke?.isTeacherTraditionalFile);
			setShowInTeacherPanelSimplifiedSeasonal(karaoke?.isTeacherSimplifiedFile);
		}
	}, [editKaraokeSeasonal]);
	/**
	 *
	...
	const getObjKaraoke = () => {
		const objKaraoke: ObjectShape = {
			[FieldNames.karaokeTitleEnglish]: stringNoSpecialChar(Errors.PLEASE_ENTER_KARAOKE_ENGLISH_TITLE),
			[FieldNames.simplifiedKaraokeLyricsFile]: stringRequired(Errors.PLEASE_UPLOAD_KARAOKE_LYRICS_FILE),
			[FieldNames.traditionalKaraokeLyricsFile]: stringRequired(Errors.PLEASE_UPLOAD_KARAOKE_LYRICS_FILE),
			[FieldNames.title]: Yup.array().of(
				Yup.object().shape({
					[FieldNames.pinyin]: stringTrim(Errors.PLEASE_ENTER_KARAOKE_PINYIN_TITLE),
					[FieldNames.traditional]: stringTrim(Errors.PLEASE_ENTER_KARAOKE_TRADITIONAL_CHINESE_TITLE),
					[FieldNames.simplified]: stringTrim(Errors.PLEASE_ENTER_KARAOKE_SIMPLIFIED_CHINESE_TITLE),
				})
			),
		};
		...
		onSubmit: (values) => {
			...
			if (editKaraokeSeasonal) {
				...
				APIService.patchData(`${URL_PATHS.karaoke}/${karaoke.uuid}`, {
					...
					isTeacherTraditionalFile: showInTeacherPanelTraditionalSeasonal,
					isTeacherSimplifiedFile: showInTeacherPanelSimplifiedSeasonal,
				})
					.then((response) => {
						if (response.status === ResponseCode.success) {
				...
				APIService.postData(`${URL_PATHS.karaoke}`, { ...values, [FieldNames.karaokeTitleEnglish]: values.karaokeTitleEnglish.trim(), isTeacherTraditionalFile: showInTeacherPanelTraditionalSeasonal, isTeacherSimplifiedFile: showInTeacherPanelSimplifiedSeasonal })
					.then((response) => {
						if (response.status === ResponseCode.success) {
							toast.success(response?.data?.message);
							formik.resetForm();
	...
	const fileUpdateSeasonalKaraoke = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
		const fileKaraoke = event.target.files?.[0] as File;
		typeValidationAudio(fileKaraoke, fileTypesVideo, Errors.UNSUPPORTED_FILE_FORMAT);
									...
									<div className='mb-4 w-full'>
										...
										<div className='mt-3'>
											<CheckBox
												option={[
													{
														id: 'karaokeShowInTeacherPanelTraditional',
														name: 'Show in teacher panel',
														value: 'Show in teacher panel',
														checked: showInTeacherPanelTraditionalSeasonal,
														onChange: () => {
															setShowInTeacherPanelTraditionalSeasonal((prev) => !prev);
														},
													},
												]}
											/>
									...
									<div className='mb-4 w-full'>
										...
										<div className='mt-3'>
											<CheckBox
												option={[
													{
														id: 'karaokeShowInTeacherPanelSimplified',
														name: 'Show in teacher panel',
														value: 'Show in teacher panel',
														checked: showInTeacherPanelSimplifiedSeasonal,
														onChange: () => {
															setShowInTeacherPanelSimplifiedSeasonal((prev) => !prev);
														},
													},
												]}
											/>
