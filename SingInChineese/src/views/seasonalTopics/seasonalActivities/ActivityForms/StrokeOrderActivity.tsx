...
const SeasonalActivityStrokeOrder = ({ onSubmit, onClose, url, activityUuid, toggleSeasonalActivity, isMoving, topicId, lessonId }: AddEditSeasonalActivityData) => {
	...
	useEffect(() => {
		if (params.activityId) {
			APIService.getData(`${url}/${activityPaths.strokeOrder}/${params.activityId}?${activityPaths.isForSeasonal}`).then((response) => {
	...
	const getObjStrokeOrderSeasonal = () => {
		const errorStrokeOrderSeasonal = Yup.string().matches(EXCLUDE_SPECIAL_CHARACTERS, Errors.SPECIAL_CHARACTERS_NOT_ALLOWED).max(CHARACTERS_LIMIT.name, Errors.MAXIMUM_LIMIT_UPTO_60_CHARACTERS);
		const obj: ObjectShape = {
			[FieldNames.leftSimplifiedEnglish]: errorStrokeOrderSeasonal,
			[FieldNames.leftTraditionalEnglish]: errorStrokeOrderSeasonal,
			[FieldNames.rightSimplifiedEnglish]: errorStrokeOrderSeasonal,
			[FieldNames.rightTraditionalEnglish]: errorStrokeOrderSeasonal,
		};
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			...
			const submitData: StrokeOrderSubmitData = {
				...
				isSkippable: provideSkipStrokeOrderSeasonal,
				isForSeasonal: true,
			};
			!params.activityId
				? APIService.postData(`${url}/${activityPaths.strokeOrderCreate}?${activityPaths.isForSeasonal}`, submitData)
						.then((response) => {
							if (response.status === ResponseCode.success) {
				...
				: APIService.putData(`${url}/${activityPaths.strokeOrderUpdate}/${params.activityId}?${activityPaths.isForSeasonal}`, submitData)
						.then((response) => {
							if (response.status === ResponseCode.success) {
								toast.success(response.data.message);
							}
	...
	const translateLeftTraditionalChineseSeasonal = useCallback(() => {
	...
	const translateRightTraditionalChineseSeasonal = useCallback(() => {
						...
						<div className='p-3'>
							...
							<div className='grid grid-cols-1 gap-x-4 p-3 bg-gray-50 rounded border'>
								<div className='flex space-x-1 items-start'>
									...
									<div className='mt-7'>
										<CommonButton data={null} dataHandler={translateLeftTraditionalChineseSeasonal} isTranslate={true} title='Translate' />
									</div>
						...
						<div className='p-3'>
							...
							<div className='grid grid-cols-1 gap-x-4 p-3 bg-gray-50 rounded border'>
								<div className='flex space-x-1 items-start'>
									...
									<div className='mt-7'>
										<CommonButton data={null} dataHandler={translateRightTraditionalChineseSeasonal} isTranslate={true} title='Translate' />
									</div>
					...
					option={[
						{
							...
							onChange: () => {
								setProvideSkipStrokeOrderSeasonal((prev) => !prev);
							},
						},
					]}
				/>
