...
const Activities = () => {
	...
	const updateActivityStatus = (editActivityData: ActivityDataArr) => {
		APIService.patchData(`${URL_PATHS.addActivity}/${editActivityData?.uuid}/${endPoint.status}?levelId=${params?.levelId}&topicId=${params.topicId}&lessonId=${params.topicId}`, {
	...
	const submitOrderActivity = useCallback(() => {
		...
		APIService.patchData(`${URL_PATHS.addActivity}/${endPoint.order}`, {
				...
				<div className='border-b p-3 flex flex-col md:flex-row items-start md:items-center justify-between'>
					<div className='mb-2 md:mb-0'>
						<h6 className='font-medium flex items-center'>
							<Button className='btn-default mr-3' onClick={pageRedirectActivity} title='Back to lessons'>
								<ArrowSmallLeft />
							</Button>
							...
							<button onClick={pageRedirectActivity}>{params.lessonName}</button>
							<span className='px-3 text-xs opacity-50'>
								<AngleRight />
							</span>
					...
					<div>
						...
						<RoleBaseGuard permissions={[permissionsArray.ACTIVITY_MANAGEMENT.AddAccess]}>
