...
export const fileTypesQA = [FILE_TYPE.audioType, FILE_TYPE.wavType];

const ActivityReadingComprehensive = ({ onSubmit, onClose, activityUuid, url, toggleActivity, isMoving, levelId, topicId, lessonId }: AddEditActivitiesData) => {
	...
	const [levelOptionsQA, setLevelOptionsQA] = useState<DropdownOptionType[]>([]);
	const [copyCheckedQA, setCopyCheckedQA] = useState<boolean>(false);
	...
	useEffect(() => {
		if (params?.activityId) {
			setLoading(true);
			APIService.getData(`${url}/${activityPaths.readingParagraph}/${params?.activityId}`)
				.then((response) => {
	...
	const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>, fileName: string) => {
		const fileQA = event.target.files?.[0] as File;
		typeValidationAudio(fileQA, fileTypesQA, Errors.PLEASE_ALLOW_MP3_WAV_FILE);
	...
	const onFileUploadHandler = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
		const fileQAAudio = event.target.files?.[0] as File;
		const fileName = event.target.name;
		typeValidationAudio(fileQAAudio, fileTypesQA, Errors.PLEASE_ALLOW_MP3_WAV_FILE);
	...
	const getObjQA = () => {
		const obj: ObjectShape = {
			...
			[FieldNames.phraseSimplified]: Yup.array().of(
				Yup.object().shape({
					[FieldNames.chinese]: recordsList.length ? stringNotRequired() : stringTrim(Errors.PLEASE_ENTER_PHRASE),
					[FieldNames.english]: recordsList.length ? stringNotRequired() : stringNoSpecialChar(Errors.PLEASE_ENTER_PHRASE),
					[FieldNames.pinyin]: recordsList.length ? stringNotRequired() : stringRequired(Errors.PLEASE_ENTER_PHRASE),
					[FieldNames.audio]: recordsList.length ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
				})
			),
			[FieldNames.audioSimplified]: audioSimplified.length ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
			...
			[FieldNames.audioTraditional]: audioTraditional.length ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
		};
		return Yup.object<ObjectShape>(obj);
	};
	const validationSchema = getObjQA();

	const sopIdQA = (value: string) => {
		return copyCheckedQA ? value : '';
	};
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			const updatedData: ReadingParagraphActivitySubmitData = {
				...
				sopLevelId: sopIdQA(values.sopLevelId as string),
			...
			if (recordsList.length) {
				if (params.activityId) {
					setLoading(true);
					APIService.putData(`${url}/${activityPaths.readingParagraph}/${params.activityId}`, { ...updatedData })
						.then((response) => {
							if (response.status === ResponseCode.success) {
					...
					setLoading(true);
					APIService.postData(`${url}/${activityPaths.readingParagraph}`, { ...updatedData, activityTypeId: values.activityTypeId })
						.then((response) => {
											...
											(arrayHelpers: MyArrayHelpers) => (
												<div className='mb-4 col-span-2'>
													{formik.values.phraseSimplified?.map((_, index) => {
														...
														return (
															<div key={`${index + 1}`} className='bg-gray-100 rounded border mb-1 p-1' draggable onDragStart={() => handleDragStart(index, FieldNames.phraseSimplified)} onDragEnd={handleDragEnd} onDragOver={(e) => handleDragOver(e, index, formik.values.phraseSimplified, FieldNames.phraseSimplified)} onDrop={handleDragEnd} style={draggedIndexSimplified === index ? { border: 'dashed ' } : {}} role='none'>
																<div className='flex space-x-1 items-start'>
																	<div className='mt-7'>
																		<span className='py-1.5 px-3 border rounded  cursor-grab active:cursor-grabbing'>
																			<Drag className='opacity-50' />
				...
				<div className='mb-4 w-96'>
					<CheckBox
						option={[
							{
								value: '',
								checked: copyCheckedQA,
								name: 'Copy this activity to SOP',
								onChange() {
									setCopyCheckedQA((prev) => !prev);
								},
							},
						]}
					/>
					{copyCheckedQA && <Dropdown label='Sop Level' placeholder='Select' name={'sopLevelId'} onChange={formik.handleChange} value={formik.values.sopLevelId} options={levelOptionsQA} id={'sopLevelId'} error={getErrorQA('sopLevelId')} required />}
				</div>
