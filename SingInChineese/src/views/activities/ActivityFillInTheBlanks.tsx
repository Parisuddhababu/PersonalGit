...
export const ActivityFillInTheBlanks = ({ onSubmit, onClose, url, activityUuid, toggleActivity, isMoving, levelId, lessonId, topicId }: AddEditActivitiesData) => {
	...
	const [correctAnswerSimplifiedBlanks, setCorrectAnswerSimplifiedBlanks] = useState<optionObject[]>([]);
	...
	const [correctAnswerTraditionalBlanks, setCorrectAnswerTraditionalBlanks] = useState<optionObject[]>([]);
	...
	const [levelOptionsBlanks, setLevelOptionsBlanks] = useState<DropdownOptionType[]>([]);
	const [copyCheckedBlanks, setCopyCheckedBlanks] = useState<boolean>(false);
	...
	useEffect(() => {
		if (params.activityId) {
			...
			APIService.getData(`${url}/${activityPaths.fillBlanks}/${params.activityId}`)
				.then((response) => {
	...
	const handleFileUploadBlanks = (event: React.ChangeEvent<HTMLInputElement>, type: string, fileName: string) => {
		const fileBlanks = event.target.files?.[0] as File;
		typeValidation(fileBlanks, type);
	...
	const getObjBlanks = () => {
		const obj: ObjectShape = {
			...
			[FieldNames.imageSimplified]: imageSimplifiedBlanks ? mixedNotRequired() : mixedRequired(Errors.IMAGE_IS_REQUIRED),
			[FieldNames.audioSimplified]: audioSimplifiedBlanks ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
			[FieldNames.optionSimplified]: simplifiedOptionalsListBlanks.length ? stringNotRequired() : stringTrim(Errors.PLEASE_ENTER_OPTION),
			[FieldNames.optionAudioSimplified]: simplifiedOptionalsListBlanks.length ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
			[FieldNames.imageTraditional]: imageTraditionalBlanks ? mixedNotRequired() : mixedRequired(Errors.IMAGE_IS_REQUIRED),
			[FieldNames.audioTraditional]: audioTraditionalBlanks ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
			[FieldNames.optionTraditional]: traditionalOptionalsListBlanks.length ? stringNotRequired() : stringTrim(Errors.PLEASE_ENTER_OPTION),
			[FieldNames.optionAudioTraditional]: traditionalOptionalsListBlanks.length ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			const updatedData: FillInTheBlanksActivitySubmitData = {
				...
				sopLevelId: copyCheckedBlanks ? values.sopLevelId : '',
				...
				traditionalAnswer: correctAnswerTraditionalBlanks,
				traditionalCorrectAnswer: correctAnswerTraditionalBlanks.map((item) => item.optionValue)?.join(' '),
				...
				simplifiedAnswer: correctAnswerSimplifiedBlanks,
				simplifiedCorrectAnswer: correctAnswerSimplifiedBlanks.map((item) => item.optionValue)?.join(' '),
			...
			if (correctAnswerSimplifiedBlanks.length && correctAnswerTraditionalBlanks.length) {
				if (params.activityId) {
					setLoadingBlanks(true);
	...
	const copyDataBlanks = useCallback(async () => {
		const traditionalBlanksOptionsTranslate = await Promise.all(
			traditionalOptionalsListBlanks.map(async (item) => {
				return {
					...item,
		...
		setSimplifiedOptionalsListBlanks(traditionalBlanksOptionsTranslate);
	 ...
	 */
	useEffect(() => {
		const updatedCorrectAnswers = correctAnswerSimplifiedBlanks?.map((item) => {
			const data = simplifiedOptionalsListBlanks?.filter((data) => data.optionId === item.optionId);
			return { optionId: data[0].optionId, optionValue: data[0].optionValue, optionFileUrl: data[0].optionFileUrl };
		});
		setCorrectAnswerSimplifiedBlanks(updatedCorrectAnswers);
	}, [simplifiedOptionalsListBlanks]);

	/**
	 * method is used to update traditional correctAnswer after edit
	 */
	useEffect(() => {
		const updatedCorrectAnswers = correctAnswerTraditionalBlanks?.map((item) => {
			const data = traditionalOptionalsListBlanks?.filter((data) => data.optionId === item.optionId);
			return { optionId: data[0].optionId, optionValue: data[0].optionValue, optionFileUrl: data[0].optionFileUrl };
		});
		setCorrectAnswerTraditionalBlanks(updatedCorrectAnswers);
	}, [traditionalOptionalsListBlanks]);

	const checkCorrectAnswerBlanks = () => {
		!correctAnswerSimplifiedBlanks.length && toast.error(Errors.PLEASE_SELECT_CORRECT_ANSWER_SIMPLIFIED);
		!correctAnswerTraditionalBlanks.length && toast.error(Errors.PLEASE_SELECT_CORRECT_ANSWER_TRADITIONAL);
	};

	const getLevelDataBlanks = () => {
						...
						<div className='rounded border w-full'>
							...
							<div className='grid grid-cols-2 gap-x-4 p-3'>
								...
								<div className='mb-4 col-span-2 overflow-auto'>
													...
													<td className='w-10 text-center'>
														<CheckBox
															option={[
																{
																	value: '',
																	checked: correctAnswerTraditionalBlanks.some((item1) => item.optionId === item1.optionId),
																	name: '',
																	onChange() {
																		const existingIndex = correctAnswerTraditionalBlanks.findIndex((data) => item.optionId === data.optionId);
																		if (existingIndex !== -1) {
																			// If the item exists, remove it
																			const updatedItems = correctAnswerTraditionalBlanks.filter((item1) => item.optionId !== item1.optionId);
																			setCorrectAnswerTraditionalBlanks(updatedItems);
																		} else {
																			// If the item doesn't exist, add it
																			const updatedItems = [...correctAnswerTraditionalBlanks, { optionId: item.optionId, optionValue: item.optionValue, optionFileUrl: item.optionFileUrl }];
																			setCorrectAnswerTraditionalBlanks(updatedItems);
																		}
																		validateFormBlanks();
																	},
															...
															disabled={editTraditionalOptionBlanks?.optionId == item.optionId}
														/>
													</td>
													<td className='w-28'>
														<div className='flex gap-3'>
															...
															<DeleteButton data={item} isDeleteStatusModal={deleteTraditionalRecordBlanks} btnDanger={true} disable={editTraditionalOptionBlanks?.optionId === item.optionId || correctAnswerTraditionalBlanks.some((item1) => item.optionId === item1.optionId)} />
														</div>
													</td>
												</tr>
											))}
									...
									<div>
										{correctAnswerTraditionalBlanks?.length !== 0 && (
											<span>
												Correct Answer : <span className='font-medium'>{correctAnswerTraditionalBlanks.map((item) => item.optionValue)?.join(' ')}</span>
											</span>
										)}
									</div>
								</div>
						...
						<div className='rounded border w-full'>
							...
							<div className='grid grid-cols-2 gap-x-4 p-3'>
								...
								<div className='mb-4 col-span-2 overflow-auto'>
													...
													<td className='w-10 text-center'>
														<CheckBox
															option={[
																{
																	value: '',
																	checked: correctAnswerSimplifiedBlanks.some((item1) => item.optionId === item1.optionId),
																	name: '',
																	onChange() {
																		const existingIndex = correctAnswerSimplifiedBlanks.findIndex((data) => item.optionId === data.optionId);
																		if (existingIndex !== -1) {
																			// If the item exists, remove it
																			const updatedItems = correctAnswerSimplifiedBlanks.filter((item1) => item.optionId !== item1.optionId);
																			setCorrectAnswerSimplifiedBlanks(updatedItems);
																		} else {
																			// If the item doesn't exist, add it
																			const updatedItems = [...correctAnswerSimplifiedBlanks, { optionId: item.optionId, optionValue: item.optionValue, optionFileUrl: item.optionFileUrl }];
																			setCorrectAnswerSimplifiedBlanks(updatedItems);
																		}
																		validateFormBlanks();
																	},
															...
															disabled={editSimplifiedOptionBlanks?.optionId == item.optionId}
														/>
													</td>
													<td className='w-28'>
														<div className='flex gap-3'>
															...
															<DeleteButton data={item} isDeleteStatusModal={deleteSimplifiedRecordBlanks} btnDanger={true} disable={editSimplifiedOptionBlanks?.optionId === item.optionId || correctAnswerSimplifiedBlanks.some((item1) => item.optionId === item1.optionId)} />
														</div>
													</td>
												</tr>
											))}
									...
									<div>
										{correctAnswerSimplifiedBlanks?.length !== 0 && (
											<span>
												Correct Answer : <span className='font-medium'>{correctAnswerSimplifiedBlanks.map((item) => item.optionValue)?.join(' ')}</span>
											</span>
										)}
									</div>
								</div>
					...
					<div className='mb-4 w-96'>
						<CheckBox
							option={[
								{
									value: '',
									checked: copyCheckedBlanks,
									name: 'Copy this activity to SOP',
									onChange() {
										setCopyCheckedBlanks((prev) => !prev);
									},
								},
							]}
						/>
						{copyCheckedBlanks && <Dropdown label='Sop Level' placeholder='Select' name={'sopLevelId'} onChange={formik.handleChange} value={formik.values.sopLevelId} options={levelOptionsBlanks} id={'sopLevelId'} error={getErrorBlanks('sopLevelId')} required />}
					</div>
