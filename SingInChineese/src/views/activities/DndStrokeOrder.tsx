...
const SortableStrokeOrderActivityItem = ({ item, editRecord, deleteStrokeOrder, editDisable }: DndStrokeOrderSortableTableItem) => {
	const { id } = item;
	...
	return (
		<tr ref={setNodeRef} style={style}>
			...
			<td className='w-40'>
				<img src={item?.simplifiedFileUrl} alt='img preview' className='border rounded w-28 min-w-[80px] mx-auto' />
			</td>
			<td className='w-16'>
		...
		<DndContext collisionDetection={closestCenter} onDragEnd={onDragEnd} modifiers={[restrictToVerticalAxis, restrictToWindowEdges]}>
			<SortableContext items={items} strategy={verticalListSortingStrategy}>
				{items?.map((item, index) => (
					<SortableStrokeOrderActivityItem key={item.id} item={item} index={index} editRecord={editRecord} deleteStrokeOrder={deleteStrokeOrder} editDisable={editDisable} />
				))}
			</SortableContext>
		</DndContext>
	);
