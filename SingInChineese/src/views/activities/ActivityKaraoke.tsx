...
import { fileTypesVideo } from './ActivityVideo';
...
const ActivityKaraoke = ({ onSubmit, onClose, editData, disabled }: ActivityKaraokeData) => {
	...
	useEffect(() => {
		if (editKaraoke) {
			...
			setShowInTeacherPanelTraditional(karaoke?.isTeacherTraditionalFile);
			setShowInTeacherPanelSimplified(karaoke?.isTeacherSimplifiedFile);
		}
	}, [editKaraoke]);
	/**
	 *
		...
		onSubmit: (values) => {
			if (editKaraoke) {
				...
				APIService.patchData(`${URL_PATHS.karaoke}/${karaoke.uuid}`, {
					...
					isTeacherTraditionalFile: showInTeacherPanelTraditional,
					isTeacherSimplifiedFile: showInTeacherPanelSimplified,
				})
					.then((response) => {
						if (response.status === ResponseCode.success) {
				...
				setLoader(true);
				APIService.postData(`${URL_PATHS.karaoke}`, { ...values, [FieldNames.karaokeTitleEnglish]: values.karaokeTitleEnglish.trim(), isTeacherTraditionalFile: showInTeacherPanelTraditional, isTeacherSimplifiedFile: showInTeacherPanelSimplified })
					.then((response) => {
						if (response.status === ResponseCode.success) {
							toast.success(response?.data?.message);
							formik.resetForm();
	...
	const fileUpdate = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
		...
		typeValidationAudio(file, fileTypesVideo, Errors.UNSUPPORTED_FILE_FORMAT);
