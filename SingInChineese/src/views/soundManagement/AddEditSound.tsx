...
import { AUDIO_SIZE_MB, FILE_TYPE } from '@config/constant';
...
import { mixedRequired, stringMaxLimit, stringNotRequired } from '@config/validations';
...
const AddEditSoundModal = ({ onSubmit, onClose, editData, disableData }: AddEditSoundManagement) => {
	...
	const getObjSound = () => {
		const obj: ObjectShape = {
			[FieldNames.name]: stringMaxLimit(Errors.PLEASE_ENTER_EVENT_NAME).matches(ONLY_ALPHABETS, Errors.NUMBER_AND_SPECIAL_CHARACTERS_NOT_ALLOWED),
			[FieldNames.audio]: editData
				? stringNotRequired()
				: mixedRequired(Errors.AUDIO_IS_REQUIRED)
