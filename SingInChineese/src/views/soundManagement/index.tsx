...
const SoundManagement = () => {
	...
	const getSoundList = useCallback(() => {
		setLoaderSound(true);
		APIService.getData(`${URL_PATHS.sound}/${endPoint.list}?page=${filterSoundData.page}&limit=${filterSoundData.limit}&eventName=${filterSoundData.search}&sortBy=${filterSoundData.sortBy}&sortOrder=${filterSoundData.sortOrder}`)
			.then((response) => {
		...
		<div>
			<div className='mb-3 bg-white shadow-lg rounded-lg overflow-auto'>
				<div className='border-b p-3 flex items-center justify-between'>
					...
					<RoleBaseGuard permissions={[permissionsArray.SOUND_MANAGEMENT.AddAccess]}>
										...
										return (
											<th scope='col' key={colVal.name}>
												<div className='flex items-center'>
													{colVal.name}
													{colVal.sortable && (
														<button onClick={() => onHandleSortSound(colVal.fieldName)}>
								...
								{soundData?.data?.data?.map((data, index: number) => {
									return (
										...
										>
											...
											<td className='w-48'>
												<audio src={data.eventSoundFile} controls>
													<track kind='captions' />
												</audio>
											</td>
											...
											<td className='cursor-default' onClick={rowClickHandler}>
												<div className='flex items-center justify-center'>
													<RoleBaseGuard permissions={[permissionsArray.SOUND_MANAGEMENT.EditAccess]}>
														<EditButton data={data} editRecord={editSoundHandler} />
													</RoleBaseGuard>
													<div className='hidden'>
														<RoleBaseGuard permissions={[permissionsArray.SOUND_MANAGEMENT.DeleteAccess]}>
															<DeleteButton data={data} isDeleteStatusModal={deleteSoundModal} />
														</RoleBaseGuard>
													</div>
													<RoleBaseGuard permissions={[permissionsArray.SOUND_MANAGEMENT.ChangeStatusAccess]}>
