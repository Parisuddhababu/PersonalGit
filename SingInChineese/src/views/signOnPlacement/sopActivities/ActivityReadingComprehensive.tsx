...
const ActivityReadingComprehensiveSop = ({ onSubmit, onClose, activityUuid, url, toggleActivity, isMoving, levelId }: AddEditActivitiesData) => {
	...
	useEffect(() => {
		if (params?.activityId) {
			...
			APIService.getData(`${url}/${activityPaths.readingParagraph}/${params?.activityId}?${activityPaths.isForSop}`)
				.then((response) => {
	...
	const handleFileUploadQASop = (event: React.ChangeEvent<HTMLInputElement>, fileName: string) => {
		const fileQASop = event.target.files?.[0] as File;
		typeValidationAudio(fileQASop, fileTypesQA, Errors.PLEASE_ALLOW_MP3_WAV_FILE);
	...
	const onFileUploadHandler = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
		const fileQASopAudio = event.target.files?.[0] as File;
		const fileName = event.target.name;
		typeValidationAudio(fileQASopAudio, fileTypesQA, Errors.PLEASE_ALLOW_MP3_WAV_FILE);
	...
	const getObjQASop = () => {
		const obj: ObjectShape = {
			...
			[FieldNames.phraseSimplified]: Yup.array().of(
				Yup.object().shape({
					[FieldNames.chinese]: recordsListQASop.length ? stringNotRequired() : stringTrim(Errors.PLEASE_ENTER_PHRASE),
					[FieldNames.english]: recordsListQASop.length ? stringNotRequired() : stringNoSpecialChar(Errors.PLEASE_ENTER_PHRASE),
					[FieldNames.pinyin]: recordsListQASop.length ? stringNotRequired() : stringRequired(Errors.PLEASE_ENTER_PHRASE),
					[FieldNames.audio]: recordsListQASop.length ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
				})
			),
			[FieldNames.audioSimplified]: audioSimplifiedQASop.length ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
			...
			[FieldNames.audioTraditional]: audioTraditionalQASop.length ? mixedNotRequired() : mixedRequired(Errors.AUDIO_IS_REQUIRED),
		};
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			...
			if (recordsListQASop.length) {
				if (params.activityId) {
					setLoadingQASop(true);
					APIService.putData(`${url}/${activityPaths.readingParagraph}/${params.activityId}?${activityPaths.isForSop}`, { ...updatedData })
						.then((response) => {
							if (response.status === ResponseCode.success) {
					...
					setLoadingQASop(true);
					APIService.postData(`${url}/${activityPaths.readingParagraph}?${activityPaths.isForSop}`, { ...updatedData, activityTypeId: values.activityTypeId, levelId: values.levelId })
						.then((response) => {
