...
import { numberRequired, stringRequired } from '@config/validations';
...
const AddEditExamModal = ({ onClose, editData, disableData }: AddEditExamResults) => {
	...
	const getUserExamDetail = (uuid: string) => {
		setLoader(true);
		APIService.getData(`${URL_PATHS.examResultDetail}/${uuid}/${endPoint.result}`)
			.then((response) => {
	...
	const getUserObj = () => {
		const obj: ObjectShape = {
			...
			[FieldNames.birthYear]: numberRequired(Errors.PLEASE_ENTER_FIRST_NAME),
		};
