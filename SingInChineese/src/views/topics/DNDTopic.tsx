...
const SortableTopicItem = ({ item, showDetails, editRecord, deleteLevelDataModal, onChangeStatus, addEditKaraoke, count }: DndTopicSortableTableItem) => {
		...
		<tr ref={setNodeRef} style={styleDndTopic} className='cursor-pointer' onClick={() => showDetails(item)}>
			...
			<td className='cursor-default' onClick={(e) => e.stopPropagation()}>
				<div className='flex justify-center items-center'>
					...
					<RoleBaseGuard permissions={[permissionsArray.TOPIC_MANAGEMENT.EditAccess]}>
						<EditButton data={item} editRecord={editRecord} />
					</RoleBaseGuard>
					<RoleBaseGuard permissions={[permissionsArray.TOPIC_MANAGEMENT.DeleteAccess]}>
						<DeleteButton data={item} isDeleteStatusModal={deleteLevelDataModal} />
					</RoleBaseGuard>
					<RoleBaseGuard permissions={[permissionsArray.TOPIC_MANAGEMENT.ChangeStatusAccess]}>
		...
		<DndContext collisionDetection={closestCenter} onDragEnd={onDragEnd} modifiers={[restrictToVerticalAxis, restrictToWindowEdges]}>
			<SortableContext items={items} strategy={verticalListSortingStrategy}>
				{items?.map((item, index) => (
					<SortableTopicItem key={item.id} item={item} showDetails={showDetails} index={index} editRecord={editRecord} deleteLevelDataModal={deleteLevelDataModal} onChangeStatus={onChangeStatus} addEditKaraoke={addEditKaraoke} count={items.length} />
				))}
			</SortableContext>
		</DndContext>
	);
