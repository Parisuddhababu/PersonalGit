...
import { stringNotRequired, stringRequired } from '@config/validations';
...
const AddEditSubAdminModal = ({ onClose, editData, onSubmit, disabled }: AddEditSubAdmin) => {
	...
	const getRoleDataListDropdown = () => {
		setLoader(true);
		APIService.getData(`${URL_PATHS.role}/${endPoint.roleList}`)
			.then((response) => {
	...
	const getObj = () => {
		const obj: ObjectShape = {
			[FieldNames.firstName]: stringRequired(Errors.PLEASE_ENTER_FIRST_NAME),
			[FieldNames.lastName]: stringRequired(Errors.PLEASE_ENTER_LAST_NAME),
			[FieldNames.email]: Yup.string().email(Errors.PLEASE_ENTER_VALID_EMAIL).required(Errors.PLEASE_ENTER_YOUR_REGISTERED_EMAIL),
			[FieldNames.roleId]: stringRequired(Errors.PLEASE_SELECT_ROLE),
			[FieldNames.password]: editData ? stringNotRequired() : Yup.string().required(Errors.PLEASE_ENTER_PASSWORD).matches(PASSWORD_REGEX, Errors.PASSWORD_MUST_CONTAIN_COMBINATION_CHARACTERS),
