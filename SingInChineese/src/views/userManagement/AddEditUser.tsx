...
import { mixedRequired, stringRequired, stringTrim } from '@config/validations';
...
const AddEditUserModal = ({ onClose, editData, disableData, onSubmit }: AddEditUser) => {
	...
	const getUserObj = () => {
		const obj: ObjectShape = {
			[FieldData.firstName]: stringTrim(Errors.PLEASE_ENTER_FIRST_NAME),
			[FieldData.lastName]: stringTrim(Errors.PLEASE_ENTER_LAST_NAME),
			[FieldData.phoneNumber]: stringTrim(Errors.PLEASE_ENTER_CONTACT_NUMBER),
			[FieldData.phoneCode]: stringRequired(Errors.PLEASE_SELECT_PHONE_CODE),
			[FieldData.dateOfBirth]: mixedRequired(Errors.PLEASE_ENTER_DATE_OF_BIRTH).test('minYear', Errors.PLEASE_ENTER_DATE_OF_BIRTH_MAX, (value) => value && value < new Date().getFullYear() - 18 && value > new Date().getFullYear() - 99),
		};
					...
					<div className={cn(ModelStyle['model__body'])}>
						<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-2'>
							<div className='mb-4 sm:col-span-2 lg:col-span-3'>
								...
								<img src={userData?.userDetail.profileImage !== '' ? userData?.userDetail.profileImage : defaultUser} alt='Img Preview' className='object-contain w-40 h-40 mb-5 mr-4 p-1 rounded-md border' />
							</div>
							<div className='mb-4'>
							...
							{!userData?.children.record ? (
								...
								<div className='col-span-3'>
									<table>
										...
										<tbody>
											{userData?.children.data.map((item, index: number) => {
												return (
													<tr key={item.id}>
														...
														<td>{item.fullName}</td>
														<td className='w-20 text-center'>{item.gender}</td>
														<td className='w-40 text-center'>{getDateFromUTCstamp(item.birthDate)}</td>
													</tr>
												);
											})}
										</tbody>
