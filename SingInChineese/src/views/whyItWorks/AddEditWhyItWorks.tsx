...
import { FILE_TYPE, fileTypeEnum, fileTypes, imageType, videoType, acceptAll } from '@config/constant';
...
import { FieldNames } from '@views/onBoarding/AddEditOnboarding';
...
const AddEditWhyItWorksModal = ({ onSubmit, onClose, editData, disableData }: AddEditOnboardingData) => {
	...
	useEffect(() => {
		if (editData) {
			...
			APIService.getData(`${URL_PATHS.onboarding}/${editData?.uuid}?isForOnboarding=false`)
				.then((response) => {
	...
	const initialValues: CreateOnboardingData = {
		...
		[FieldNames.isForOnboarding]: false,
	};
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			if (checkImageOrVideoWhyItWorks) {
				...
				if (editData) {
					setLoaderWhyItWorks(true);
					APIService.patchData(`${URL_PATHS.onboarding}/${editData.uuid}`, commonData)
						.then((response) => {
							if (response.status === ResponseCode.success) {
					...
					setLoaderWhyItWorks(true);
					APIService.postData(URL_PATHS.onboarding, commonData)
						.then((response) => {
