...
import { ArrowSmallLeft, CreditCard, Search } from '@components/icons';
...
const FlashCard = () => {
	...
	const getFlashCardDetails = useCallback(() => {
		...
		APIService.getData(`${URL_PATHS.flashCard}/${endPoint.list}?categoryId=${params.id}&page=${filterFlashcardData.page}&limit=${filterFlashcardData.limit}&sortBy=${filterFlashcardData.sortBy}&sortOrder=${filterFlashcardData.sortOrder}&search=${filterFlashcardData.search}`)
			.then((response) => {
	...
	const updateCategoryStatus = (value: boolean) => {
		setLoadingFlashcard(true);
		APIService.patchData(`${URL_PATHS.flashCard}/${editData?.uuid}/${endPoint.changeStatus}`, {
			isActive: value,
		})
			.then((response) => {
				if (response.status === ResponseCode.success) {
										...
										return (
											<th scope='col' key={colVal.name}>
												<div className='flex items-center'>
													{colVal.name}
													{colVal.sortable && (
														<button onClick={() => onFlashcardHandleSort(colVal.fieldName)}>
								...
								{flashCardData?.data.flashcards.map((data, index) => {
									return (
										<tr key={data.uuid}>
											...
											<td>
												<RoleBaseGuard permissions={[permissionsArray.CATEGORY_FLASHCARD_MANAGEMENT.ChangeStatusAccess]}>
