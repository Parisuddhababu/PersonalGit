...
const Level = () => {
	...
	const getCountryData = useCallback(() => {
		setLoader(true);
		APIService.getData(`${URL_PATHS.country}/${endPoint.list}?page=${filterCountryData.page}&limit=${filterCountryData.limit}&search=${filterCountryData.search}&sortBy=${filterCountryData.sortBy}&sortOrder=${filterCountryData.sortOrder}`)
			.then((response) => {
		...
		<div>
			...
			<div className='mb-3 bg-white shadow-lg rounded-lg overflow-auto'>
				<div className='border-b p-3 flex items-center justify-between'>
					...
					<RoleBaseGuard permissions={[permissionsArray.COUNTRY_MANAGEMENT.AddAccess]}>
										...
										return (
											<th scope='col' key={colVal.name}>
												<div className='flex items-center'>
													{colVal.name}
													{colVal.sortable && (
														<button onClick={() => onCountryHandleSort(colVal.fieldName)}>
															{(filterCountryData.sortOrder === '' || filterCountryData.sortBy !== colVal.fieldName) && getDefaultIcon()}
															{filterCountryData.sortOrder === sortOrderValues.asc && filterCountryData.sortBy === colVal.fieldName && getAscIcon()}
															{filterCountryData.sortOrder === sortOrderValues.desc && filterCountryData.sortBy === colVal.fieldName && getDescIcon()}
														</button>
													)}
												</div>
											</th>
										...
										>
											...
											<td className='cursor-default' onClick={rowClickHandler}>
												<div className='flex justify-center items-center'>
													<RoleBaseGuard permissions={[permissionsArray.COUNTRY_MANAGEMENT.EditAccess]}>
														<EditButton data={data} editRecord={editRecord} />
													</RoleBaseGuard>
													<RoleBaseGuard permissions={[permissionsArray.COUNTRY_MANAGEMENT.DeleteAccess]}>
														<DeleteButton data={data} isDeleteStatusModal={deleteCountryDataModal} />
													</RoleBaseGuard>
													<RoleBaseGuard permissions={[permissionsArray.COUNTRY_MANAGEMENT.ChangeStatusAccess]}>
