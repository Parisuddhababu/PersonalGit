...
const BVDataTable = ({ columns, queryName, singleDeleteMutation, multipleDeleteMutation, updateStatusMutation, sessionFilterName, updatedFilterData, actionWisePermissions, defaultActions, actionData, extraActions, statusKey, idKey, multipleDeleteApiId, singleDeleteApiId, statusChangeApiId, statusChangeApiKeyTitle, rowRefData }: IBVDataTablesProps): ReactElement => {
	...
	/**
	 * Singe Delete, Multiple Delete, Change Status Mutation
	 */
	const [updateStatusData, { loading: statusLoader }] = updateStatusMutation ? useMutation(updateStatusMutation) : [null, { loading: false }];
	const [deleteSingle, { loading: deleteSingleLoader }] = singleDeleteMutation ? useMutation(singleDeleteMutation) : [null, { loading: false }];
	const [deleteMultiple, { loading: deleteMultipleLoader }] = multipleDeleteMutation ? useMutation(multipleDeleteMutation) : [null, { loading: false }];
	...
	}, [isDeletePopup, singleDeleteId]);

	/**
	 * Change ths status of items
	 */
	const changeStatusAction = useCallback(() => {
		if (statusChangeApiId && updateStatusMutation && updateStatusData) {
			updateStatusData({
				variables: {
	...
	}, [isStatusPopup, currentStatus, statusChangeId]);
	...
	return (
		<>
			{deleteMultipleLoader || deleteSingleLoader || (statusLoader && <LoadingIndicator />)}
			<div className='table-select-dropdown-container md:justify-between justify-start'>
						...
						{listData?.map((row) => {
							return (
								<tr className='text-left' key={row?.[`${idKey}`] ?? row.uuid}>
									...
									{columns.map((column) => (
										<td key={column.name}>
											{column.type === 'image' && <img src={`${IMAGE_BASE_URL}${row?.[column.fieldName]?.original_file}`} alt={column.fieldName} className='w-wide-3 m-auto cursor-pointer' onClick={() => openImageModel(`${IMAGE_BASE_URL}${row?.[column.fieldName]?.original_file}`)} />}
											{column.type === 'date' && getDateFromat(row?.[column.fieldName], DATE_FORMAT.momentDateTime24Format)}
											{column.type === 'text' && getApiColumnName(column.fieldName, row)}
														...
														<RoleBaseGuard permissions={[actionWisePermissions?.changeStatus ?? '']}>
															<div title={t('Change Status') ?? ''} className='flex justify-center'>
																<span onClick={() => statusPopup(row?.[`${idKey}`], row?.[`${statusKey}`])} className='font-medium text-blue-600 mt-2 hover:underline'>
																	<label title={t(AccesibilityNames.ChangeStatus).toString()} className='relative inline-flex items-center cursor-pointer'>
