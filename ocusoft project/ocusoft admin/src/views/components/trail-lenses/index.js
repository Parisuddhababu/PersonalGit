...
const TrailLensesOrdersList = () => {
  ...
  const [trailOrdersList, setTrailOrdersList] = useState([]);
  ...
  const filterMap = {
    ...
    sap_order_id: getSapOrderId(),
    order_status: getOrderStatus(),
  };
  /*to fetch trails order acoount list */
  const getHcpList = () => {
    API.getMasterList(hcpListResponseObj, null, true, Constant.ACCOUNT_LIST);
  };
  ...
  /*to fetch trails orders table list */
      ...
      if (response?.meta?.status) {
        ...
        const _orders = responseData.map((orderObj) => {
          return {
            ...
            status:
              orderStatusDirectory?.[+orderObj?.order_status ?? 1] ?? // NOSONAR
              "In process",
            paymentStatus:
              orderObj?.transactions?.payment_status_text ?? "PAID",
          };
        });
        setTrailOrdersList([..._orders]);
  ...
  const handleOrderSearchParamChange = (e) => {
    const { name, value } = e?.target; // NOSONAR
    setSearchParams({ ...searchParams, [name]: value });
  };
  /*funtion to navigate view trail orders page*/
  const moveToDetailsPage = (id) => {
    if (id) history.push(`trial-lenses-orders/view/?id=${id}`);
    else showError("order id is unavailable.");
  };
  /*funtion to navigate add trail orders page */
  const moveToOrderAddPage = (e) => {
    e.preventDefault();
    history.push("trial-lenses-orders/add");
  };
  /*funtions for body template*/
  ...
  const actionBodyTemplate = (rowData) => {
    return (
      <>
        ...
        <a
          title="Sync"
          className="mr-2 text-success" // NOSONAR
          onClick={() => {
  ...
  const handleGetOrderListInCSVResponseObj = {
    ...
    success: (response) => {
      ...
      if (fileUrl) downloadFile(fileUrl, "trail_orders.xlsx");
  ...
  const handleDownload = (e) => {
    e.preventDefault();
    const { hcp, number, sapId, status } = searchParams;
    const data = hcp
      ? {
          account_id: hcp,
          order_number: number,
          sap_order_id: sapId,
          order_status:
            orderStatusDirectory.indexOf(status) !== -1
              ? orderStatusDirectory.indexOf(status)
              : null,
        }
      : null;
    setIsLoading(true);
    ...
    <div className="table-header">
      <div className="clearfix">
        <h5 className="p-m-0 float-start">
          ...
          {CommonMaster.TRAIL_ORDERS_LIST}
    ...
    <>
      ...
      <div className="datatable-responsive-demo custom-react-table">
        <div className="card">
          <DataTable
            stripedRows
            showGridlines
            value={trailOrdersList}
            responsiveLayout="scroll"
            header={ordersHeaderTemplate}
            className="p-datatable-responsive-demo"
            footer={trailOrdersList?.length > 0 ? ordersFooterTemplate : <></>}
          >
            {adminRole === "SUPER_ADMIN" && (
              <Column header="HCP" field="hcpName" body={hcpNameBodyTemplate} />
            )}
            {
              <Column
                field="number"
      ...
      {showCancelModal && (
        <CancelModal
          ...
          apiType="Trail_orders"
        />
      )}
    </>
  );
};
export default TrailLensesOrdersList;
