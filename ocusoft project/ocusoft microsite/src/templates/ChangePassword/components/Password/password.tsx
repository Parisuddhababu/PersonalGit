...
import { IChangePasswordForm } from "@templates/ChangePassword/components/Password/index";
import { useRouter } from "next/router";
import Head from "next/head";
import { CSS_NAME_PATH } from "@constant/cssNamePath.constant";
import { getTypeBasedCSSPath, removeLoginData } from "@util/common";
...
const ChangePasswordSection1 = () => {
  ...
  const onSubmit: SubmitHandler<IChangePasswordForm> = async (data) => {
    try {
      ...
      if (
        responseData?.meta?.status_code === 201 ||
        responseData?.meta?.status_code === 200
      ) {
        clearLoginInData();
